...
object Measure {
     ...
     *
     * Note: Use this method when you need to trace an operation that has already started and you have
     * captured its start time using [getCurrentTime], with explicit control over span hierarchy using [setNoParent].
     */
    fun startSpan(name: String, timestamp: Long, setNoParent: Boolean): Span {
        if (isInitialized.get()) {
            return measure.startSpan(name, timestamp = timestamp, setNoParent = setNoParent)
        } else {
            return Span.invalid()
        }
    }
