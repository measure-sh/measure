...
object Measure {
    ...
    /**
     * Puts the [span] in scope. Putting in scope means putting the span in thread local. Any spans
     * created on this thread will now have [span] set as it's parent automatically.
     *
     * The SDK must be initialized, otherwise the span will not be set in scope, the block
     * will continue to run normally.
     *
     * When in scope, the [getCurrentSpan] function will return this [span].
     */
    fun <T> withScope(span: Span?, block: () -> T): T {
        if (isInitialized.get()) {
            if (span == null) {
                return block()
            }
