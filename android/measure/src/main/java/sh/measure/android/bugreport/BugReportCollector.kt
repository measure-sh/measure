...
internal interface BugReportCollector {
    ...
    fun startBugReportFlow(activity: Activity, takeScreenshot: Boolean = true)
...
internal class BugReportCollectorImpl internal constructor(
    ...
    override fun startBugReportFlow(activity: Activity, takeScreenshot: Boolean) {
    ...
    private fun List<Uri>.toEventAttachments(context: Context): List<EventAttachment> {
        return mapNotNull { uri ->
            try {
                ...
                logger.log(LogLevel.Error, "Unable to read file", e)
                null
            }
        }
    }
