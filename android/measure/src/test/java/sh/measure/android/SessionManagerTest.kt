...
class SessionManagerTest {
    ...
    fun `creates new session if last session crashed even if last event happened within threshold time`() {
        ...
        timeProvider.fakeCurrentTimeSinceEpochInMillis = sessionCreatedAt
        sessionManager.init()
        ...
        timeProvider.fakeCurrentTimeSinceEpochInMillis = lastEventTime
    ...
    fun `updates preferences with recent session`() {
        ...
        // Then
        Mockito.verify(prefsStorage, times(1)).setRecentSession(recentSession)
    }
    ...
    fun `creates new session when app comes back to foreground after threshold time`() {
        ...
        timeProvider.fakeCurrentTimeSinceEpochInMillis = appBackgroundedAt
        sessionManager.onAppBackground()

        // increment time
        val timeInBackground = 1000L
        timeProvider.fakeCurrentTimeSinceEpochInMillis = appBackgroundedAt + timeInBackground
