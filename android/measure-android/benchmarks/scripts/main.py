...
TEST_OPTION = typer.Option(
    None,
    "--test",
    help="Case-insensitive substring matched against className or name to "
         "pick a benchmark entry. Required when the JSON has more than one.",
)


@app.command()
def compare_startup(path_before: str, path_after: str, test: Optional[str] = TEST_OPTION):
    before_data = parse_benchmark.startup(path_before, test)
    after_data = parse_benchmark.startup(path_after, test)
    calculate_and_print_metrics(before_data, after_data)


@app.command()
def compare_track_gesture(path_before: str, path_after: str, test: Optional[str] = TEST_OPTION):
    before_data = parse_benchmark.track_gesture(path_before, test)
    after_data = parse_benchmark.track_gesture(path_after, test)
    calculate_and_print_metrics(before_data, after_data)


@app.command()
def compare_click_heap(path_before: str, path_after: str, test: Optional[str] = TEST_OPTION):
    before_data = parse_benchmark.click_heap(path_before, test)
    after_data = parse_benchmark.click_heap(path_after, test)
    calculate_and_print_metrics(before_data, after_data)
