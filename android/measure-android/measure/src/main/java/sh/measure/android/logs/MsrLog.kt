 ...
 * SDK is initialized. When present, the tag prefixes the body as `tag: message`. Any
 * [Throwable] argument is ignored (only the message is logged); exceptions should be
 * reported via the exception APIs. The minimum severity collected is governed by the SDK
 * config.
 */
object MsrLog {
    @JvmStatic
    ...
    fun println(priority: Int, tag: String?, msg: String): Int {
        ...
        track(priority, tag, msg)
        return result
    }

    private fun track(priority: Int, tag: String?, msg: String?) {
        val collector = Measure.getLogEventCollector() ?: return
        track(collector, priority, tag, msg)
    }
    ...
    internal fun track(
        ...
        if (!collector.isAutomaticCollectionEnabled) {
            return
        }
        if (msg.isNullOrEmpty()) {
            return
        }
        val body = if (tag.isNullOrEmpty()) msg else "$tag: $msg"
        collector.trackLog(
            body = body,
