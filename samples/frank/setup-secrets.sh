...
{
  echo "measure.debug.apiKey=${FRANK_MEASURE_API_KEY_DEBUG:-}"
  echo "measure.debug.apiUrl=${FRANK_MEASURE_API_URL_DEBUG:-}"
  echo "measure.release.apiKey=${FRANK_MEASURE_API_KEY_RELEASE:-}"
  echo "measure.release.apiUrl=${FRANK_MEASURE_API_URL_RELEASE:-}"
} >> "$LOCAL_PROPS"
echo "Wrote Measure config to local.properties"

# iOS: MeasureDebug.xcconfig
ESCAPED_DEBUG_URL=$(echo "${FRANK_MEASURE_API_URL_DEBUG:-}" | sed 's|//|$()/$()/|g')
printf '#include? "Generated.xcconfig"\n\nMEASURE_API_KEY = %s\nMEASURE_API_URL = %s\n' \
  "${FRANK_MEASURE_API_KEY_DEBUG:-}" "$ESCAPED_DEBUG_URL" \
  > "$SCRIPT_DIR/ios/MeasureDebug.xcconfig"
echo "Wrote ios/MeasureDebug.xcconfig"

# iOS: MeasureRelease.xcconfig
ESCAPED_RELEASE_URL=$(echo "${FRANK_MEASURE_API_URL_RELEASE:-}" | sed 's|//|$()/$()/|g')
printf '#include? "Generated.xcconfig"\n\nMEASURE_API_KEY = %s\nMEASURE_API_URL = %s\n' \
  "${FRANK_MEASURE_API_KEY_RELEASE:-}" "$ESCAPED_RELEASE_URL" \
