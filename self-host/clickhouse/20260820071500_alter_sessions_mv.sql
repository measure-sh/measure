...
select
    ...
    -- a dump-only anr has no exceptions, so its subject takes the message
    -- slot, leaving the tuple's five String elements unchanged
    groupUniqArrayArraySimpleState(if(type = 'anr', [(simpleJSONExtractString(`anr.exceptions`, 'type'), if(`anr.exceptions` = '' or `anr.exceptions` = '[]', `anr.subject`, simpleJSONExtractString(`anr.exceptions`, 'message')), simpleJSONExtractString(`anr.exceptions`, 'file_name'), simpleJSONExtractString(`anr.exceptions`, 'class_name'), simpleJSONExtractString(`anr.exceptions`, 'method_name'))], [])) as unique_anrs,
