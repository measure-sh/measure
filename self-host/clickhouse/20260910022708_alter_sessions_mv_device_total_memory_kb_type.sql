...
select
    ...
    -- device_total_memory_kb is now SimpleAggregateFunction(anyLast, ...):
    -- a plain argMax(...) result merged unpredictably across background
    -- merges (observed converging to null in practice); anyLastSimpleState
    -- matches the column's declared combinator, the same way sum/min/max
    -- SimpleState calls already match theirs.
    anyLastSimpleState(attribute.device_total_memory_kb) as device_total_memory_kb,
    ...
    -- swap missing but anon_rss present: treat swap as 0 rather than drop the reading
    quantilesStateIf(0.5, 0.9, 0.95)(
        toUInt64(memory_usage_dynamic.anon_rss + coalesce(memory_usage_dynamic.swap, 0)),
        type = 'memory_usage_dynamic' and memory_usage_dynamic.anon_rss is not null
    ) as memory_usage_percentiles,
    quantilesStateIf(0.5, 0.9, 0.95)(
        memory_usage_absolute.used_memory,
        type = 'memory_usage_absolute'
    ) as memory_usage_absolute_percentiles
from events
group by
    team_id,
    app_id,
...
select
    ...
    argMax(attribute.device_manufacturer, timestamp) as device_manufacturer,
    argMax(attribute.device_model, timestamp) as device_model,
    argMax(attribute.device_total_memory_kb, timestamp) as device_total_memory_kb,
    groupUniqArrayArraySimpleState(if(attribute.user_id != '', [attribute.user_id], [])) as user_ids,
    groupUniqArrayArraySimpleState([type]) as unique_types,
    groupUniqArrayArraySimpleState(if(type = 'custom', [custom.name], [])) as unique_custom_type_names,
    groupUniqArrayArraySimpleState(if(`string.string` != '', [`string.string`], [])) as unique_strings,
