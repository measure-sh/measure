-- migrate:up
alter table events_rmt
    add column if not exists `log.severity_text` LowCardinality(FixedString(10)) comment 'log level - info, warning, error, fatal, debug' CODEC(ZSTD(3)) after `app_exit.pid`,
