...
func (t *UrlTrie) Insert(pattern UrlPattern) {
	...
	for _, segment := range pattern.Parts {
		...
		// Never collapse domain-level nodes (direct children of root).
		// Only path segments should be wildcarded.
		if current.depth > 0 && len(current.children) >= t.HighCardinalityThreshold {
			current.collapseToWildcard()
			current = current.children["*"]
			continue
		}
