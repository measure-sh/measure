...
const (
	...
	// to be kept.
	patternMinRequestCountThreshold = 50
...
func getLastReportedMetricsAt(ctx context.Context, teamID, appID uuid.UUID) (*time.Time, error) {
	stmt := sqlf.PostgreSQL.Select("metrics_reported_at").
...
// deletePatterns removes URL patterns using ALTER TABLE DELETE.
func deletePatterns(ctx context.Context, patterns []UrlPattern, teamID, appID uuid.UUID) error {
	...
	for _, p := range patterns {
		...
		if err := server.Server.ChPool.Exec(ctx, stmt.String(), stmt.Args()...); err != nil {
			stmt.Close()
			return fmt.Errorf("failed to delete pattern %s%s: %w", domain, path, err)
		}
		stmt.Close()
	}
...
func insertAggregatedMetrics(ctx context.Context, teamID, appID uuid.UUID, from, to time.Time) error {
	stmt := sqlf.
		...
		Select("groupUniqArray(e.protocol)").
		Select("groupUniqArray(e.port)").
		Select("groupUniqArray(e.method)").
		...
		Select("groupUniqArray(e.`attribute.network_type`)").
		Select("groupUniqArray(e.`attribute.network_generation`)").
		Select("groupUniqArray(e.`attribute.device_locale`)").
		Select("groupUniqArray(e.`inet.country_code`)").
		Select("toUInt64(count())").
		Select("toUInt64(countIf(e.status_code >= 200 AND e.status_code < 300))").
		Select("toUInt64(countIf(e.status_code >= 300 AND e.status_code < 400))").
		Select("toUInt64(countIf(e.status_code >= 400 AND e.status_code < 500))").
		Select("toUInt64(countIf(e.status_code >= 500 AND e.status_code < 600))").
...
func upsertMetricsReportedAt(ctx context.Context, teamID, appID uuid.UUID, reportedAt time.Time) error {
	stmt := sqlf.PostgreSQL.InsertInto("network_metrics_reporting").
		Set("team_id", teamID).
		Set("app_id", appID).
...
func GeneratePatterns(ctx context.Context) {
	...
	for _, t := range teams {
		...
		for _, app := range apps {
			...
			existing, err := fetchExistingPatterns(ctx, app.TeamID, app.ID)
			if err != nil {
			...
			// insert patterns
			if err := insertPatterns(ctx, insert, app.TeamID, app.ID); err != nil {
				fmt.Printf("failed to insert for team=%s app=%s: %v\n",
					app.TeamID, app.ID, err)
				continue
			}

			// delete patterns that were collapsed by the trie
			...
			if err := deletePatterns(ctx, delete, app.TeamID, app.ID); err != nil {
	...
	for _, t := range teams {
		...
		for _, app := range apps {
			...
			if err := insertAggregatedMetrics(ctx, app.TeamID, app.ID, *from, now); err != nil {
				fmt.Printf("failed to insert c for team=%s app=%s: %v\n",
					app.TeamID, app.ID, err)
				continue
			}
