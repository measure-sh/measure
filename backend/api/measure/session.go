...
// GetSessionsInstancesPlot provides aggregated session instances
// matching various filters.
func GetSessionsInstancesPlotTwo(ctx context.Context, af *filter.AppFilter) (sessionInstances []session.SessionInstance, err error) {
	...
	base.Where("first_event_timestamp >= ? and last_event_timestamp <= ?", af.From, af.To)
	...
	if af.Crash && af.ANR {
		base.Where("sumMerge(crash_count) >= 1 or sumMerge(anr_count) >= 1")
	} else if af.Crash {
		base.Having("sumMerge(crash_count) >= 1")
	} else if af.ANR {
		base.Having("sumMerge(anr_count) >= 1")
	...
	stmt := sqlf.
		...
		Select("uniq(session_id) instances").
		Select("formatDateTime(start_time, '%Y-%m-%d', ?) datetime", af.Timezone).
		Select("concat(app_version.1, ' ', '(', app_version.2, ')') app_version_fmt").
		GroupBy("app_version, datetime").
		OrderBy("datetime, app_version.2 desc")
...
// GetSessionsWithFilter provides sessions that matches various
// filter criteria in a paginated fashion.
func GetSessionsWithFilterTwo(ctx context.Context, af *filter.AppFilter) (sessions []SessionDisplay, next, previous bool, err error) {
	teamId, err := ambient.TeamId(ctx)
	if err != nil {
	...
	if af.Limit > 0 {
		base.Limit(uint64(af.Limit) + 1)
	}

	if af.Offset >= 0 {
		base.Offset(uint64(af.Offset))
	}

	// Don't return boring sessions that has less than n events, so filter
	// those out. Many sessions may have just a `session_start`
	// event & nothing else.
	base.Having("sumMerge(event_count) >= ?", sessionMinEvents)
	...
	if af.HasVersions() {
		...
		// stmt.Where("app_version in (?)", selectedVersions.Parameterize())
		base.Where("app_version_major in ?", af.Versions)
		base.Where("app_version_minor in ?", af.VersionCodes)
	}
	...
	base.GroupBy("session_id, app_version, device_name, device_model, device_manufacturer, os_version")
	...
	stmt := sqlf.With("base", base).
		...
		Select("end_time").
		GroupBy("session_id").
		GroupBy("app_version_major").
		GroupBy("app_version_minor").
		GroupBy("os_version_major").
		GroupBy("os_version_minor").
		GroupBy("device_name").
	...
	for rows.Next() {
		var uniqueUserIds, uniqueTypes, uniqueStrings, uniqueViewClassnames, uniqueSubviewClassnames []string
		uniqueExceptions := []map[string]string{}
		uniqueANRs := []map[string]string{}
		uniqueClickTargets := []map[string]string{}
		uniqueLongclickTargets := []map[string]string{}
		...
		if af.FreeText != "" {
			dest = append(dest, &uniqueUserIds, &uniqueTypes, &uniqueStrings, &uniqueViewClassnames, &uniqueSubviewClassnames, &uniqueExceptions, &uniqueANRs, &uniqueClickTargets, &uniqueLongclickTargets, &uniqueScrollTargets)
		}
