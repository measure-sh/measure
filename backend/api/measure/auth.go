...
func SigninGitHub(c *gin.Context) {
	...
	case "code":
		...
		authsession.SetCookies(c, authSess.AccessToken, authSess.RefreshToken,
			authSess.AccessTokenExpiryAt, authSess.RefreshTokenExpiryAt)
...
func RefreshToken(c *gin.Context) {
	...
	newSession, err := authsession.NewAuthSession(oldSession.UserID, oldSession.OwnTeamId, oldSession.OAuthProvider, oldSession.UserMeta)
	if err != nil {
		fmt.Println(msg, err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": msg,
	...
	authsession.SetCookies(c, newSession.AccessToken, newSession.RefreshToken,
		newSession.AccessTokenExpiryAt, newSession.RefreshTokenExpiryAt)
