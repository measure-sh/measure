...
func TestPaidConversion(t *testing.T) {
	...
	t.Run("new subscription to Pro fires paid_conversion with correct payload", func(t *testing.T) {
		...
		payload := []byte(fmt.Sprintf(
			`{"type":"customer.products.updated","data":{"scenario":"new","customer":{"id":%q},"updated_product":{"id":%q,"started_at":%d}}}`,
			custID, autumnPlanPro, startedAt,
		))
		...
		wantTxn := fmt.Sprintf("%s:%s:%d", custID, autumnPlanPro, startedAt)
	...
	t.Run("new subscription to Free does not fire paid_conversion", func(t *testing.T) {
		...
		// Skip-free branch fires before applyPlanTransition, so no autumn mock needed.

		payload := []byte(fmt.Sprintf(
			`{"type":"customer.products.updated","data":{"scenario":"new","customer":{"id":%q},"updated_product":{"id":%q,"started_at":1700000000000}}}`,
