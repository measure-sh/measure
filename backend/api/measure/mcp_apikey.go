...
func revokeMcpKey(ctx context.Context, mcpKeyId uuid.UUID) error {
	...
	_, err := server.Server.PgPool.Exec(ctx, stmt.String(), true, mcpKeyId)
	return err
}
...
func DecodeMcpKey(key string) (*uuid.UUID, *uuid.UUID, error) {
	...
	if err := server.Server.PgPool.QueryRow(context.Background(), stmt.String(), stmt.Args()...).Scan(&userId, &teamId); err != nil {
