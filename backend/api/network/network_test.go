...
func TestFetchDomains(t *testing.T) {
	...
	t.Run("returns domains ordered by count DESC", func(t *testing.T) {
		...
		domains, err := FetchDomains(ctx, appID, teamID)
		if err != nil {
	...
	t.Run("events older than 30 days are excluded", func(t *testing.T) {
		cleanupAll(ctx, t)
		defer cleanupAll(ctx, t)
		...
		seedHttpEvent(ctx, t, teamID.String(), appID.String(), "https://new-domain.com/path", "GET", 200, 5, new)
		...
		if len(domains) > 1 {
			t.Errorf("expected 1 domains, got %d: %v", len(domains), domains)
		}
	})
}
...
func TestFetchPaths(t *testing.T) {
	...
	t.Run("returns paths from url_patterns ordered by path", func(t *testing.T) {
		...
		paths, err := FetchPaths(ctx, appID, teamID, "api.example.com", "")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if len(paths) != 3 {
	...
	t.Run("search filter on patterns", func(t *testing.T) {
		...
		paths, err := FetchPaths(ctx, appID, teamID, "api.example.com", "user")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if len(paths) != 1 {
	...
	t.Run("fallback to http_events when no patterns match search", func(t *testing.T) {
		...
		paths, err := FetchPaths(ctx, appID, teamID, "api.example.com", "fallback")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if len(paths) != 1 {
	...
	t.Run("no results returns empty", func(t *testing.T) {
		...
		paths, err := FetchPaths(ctx, appID, teamID, "nonexistent.com", "")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if paths != nil {
...
func TestFetchOverviewTimeline(t *testing.T) {
	ctx := context.Background()
	cleanupAll(ctx, t)
	defer cleanupAll(ctx, t)
	...
	resp, err := FetchOverviewTimeline(ctx, appID, teamID, af)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestFetchEndpointTimeline(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("returns timeline for existing pattern", func(t *testing.T) {
		...
		resp, err := FetchEndpointTimeline(ctx, appID, teamID, "api.example.com", "/v1/users", af)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if resp == nil {
	...
	t.Run("non-existent pattern returns nil", func(t *testing.T) {
		...
		resp, err := FetchEndpointTimeline(ctx, appID, teamID, "nonexistent.com", "/v1/nope", af)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if resp != nil {
