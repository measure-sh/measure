...
func FetchPaths(ctx context.Context, appId, teamId uuid.UUID, domain, search string) (paths []string, err error) {
	...
	// Fallback to http_events if not patterns found
...
func FetchOverviewTimeline(ctx context.Context, appId, teamId uuid.UUID, af *filter.AppFilter) (*TimelineResponse, error) {
	stmt := sqlf.
		Select("domain").
		Select("path").
