...
func FetchTrends(ctx context.Context, appId, teamId uuid.UUID, af *filter.AppFilter, limit int) (*TrendsResponse, error) {
	...
	trendsGroup.Go(func() (err error) {
		result.TrendsLatency, err = fetchTrendsCategory(ctx, appId, teamId, af, "p95_latency DESC", limit)
		return
	})

	trendsGroup.Go(func() (err error) {
		result.TrendsErrorRate, err = fetchTrendsCategory(ctx, appId, teamId, af, "error_rate DESC", limit)
		return
	})

	trendsGroup.Go(func() (err error) {
		result.TrendsFrequency, err = fetchTrendsCategory(ctx, appId, teamId, af, "frequency DESC", limit)
		return
	})
...
func FetchOverviewTimelinePlot(ctx context.Context, appId, teamId uuid.UUID, af *filter.AppFilter, limit int) (*TimelineResponse, error) {
	if limit <= 0 {
...
func FetchEndpointTimelinePlot(ctx context.Context, appId, teamId uuid.UUID, domain, pathPattern string, af *filter.AppFilter) (*TimelineResponse, error) {
