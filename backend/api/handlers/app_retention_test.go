...
func TestUpdateAppRetention(t *testing.T) {
	...
	t.Run("billing enabled → 403 with plan-driven message", func(t *testing.T) {
		...
		if got := getAppRetention(ctx, t, appID); got != measure.MIN_RETENTION_DAYS {
			t.Errorf("retention changed despite 403: got %d, want %d", got, measure.MIN_RETENTION_DAYS)
		}
	})
	...
	t.Run("self-hosted, retention above max → 400", func(t *testing.T) {
		...
		c, w := newTestGinContext("PATCH", "/apps/"+appID.String()+"/retention", bytes.NewReader(retentionPayload(measure.MAX_RETENTION_DAYS+1)))
		c.Set("userId", userID)
		c.Params = gin.Params{{Key: "id", Value: appID.String()}}
