...
func (flt *Filter) Predicate(keyBindingOverrides map[string]KeyBinding) (*sqlf.Stmt, error) {
	bindLeaf := func(condition Condition) (*sqlf.Stmt, error) {
		...
		return flt.Entity.BindKey(condition)
	}
	...
	isCustom := func(condition Condition) bool {
		return flt.customBinder != nil && strings.HasPrefix(condition.KeyName, CustomKeyPrefix)
	}
	...
	walk = func(node *ExprTree) (*sqlf.Stmt, error) {
		if !node.IsGroup() {
			if isCustom(*node.Condition) {
				return flt.customBinder(LogicalAnd, []Condition{*node.Condition})
			}
		...
		if len(customBatch) > 0 {
			converted, err := flt.customBinder(node.LogicalOperator, customBatch)
			if err != nil {
				return nil, err
			}
			children = append(children, converted)
