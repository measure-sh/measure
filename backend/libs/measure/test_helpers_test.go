...
// which live in each service's server package. libs tests can't import a
// service, so they hold just the handles + config values the logic under test
// needs.
...
func (c *testConfig) IsBillingEnabled() bool { return c.BillingEnabled }
