...
func (j *JourneyAndroid) SetExceptionGroups(ctx context.Context, rch driver.Conn, af *filter.AppFilter) (err error) {
	fingerprints := make([]string, 0, len(j.exceptionGroups))
	for k := range j.exceptionGroups {
		fingerprints = append(fingerprints, k)
	}

	groups, err := group.GetExceptionGroupsFromFingerprints(ctx, rch, af, fingerprints)
	if err != nil {
		return
	}
...
func (j *JourneyAndroid) SetANRGroups(ctx context.Context, rch driver.Conn, af *filter.AppFilter) (err error) {
	fingerprints := make([]string, 0, len(j.anrGroups))
	for k := range j.anrGroups {
		fingerprints = append(fingerprints, k)
	}

	groups, err := group.GetANRGroupsFromFingerprints(ctx, rch, af, fingerprints)
	if err != nil {
		return
	}
