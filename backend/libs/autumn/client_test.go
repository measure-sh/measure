...
func TestTrackTokens(t *testing.T) {
	cleanup := setupTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost || r.URL.Path != "/v1/balances.track_tokens" {
			t.Errorf("unexpected request: %s %s", r.Method, r.URL.Path)
		}
		body, _ := io.ReadAll(r.Body)
		...
		if got["customer_id"] != "team_123" || got["model_id"] != "openrouter/deepseek/deepseek-v4-pro" ||
			got["input_tokens"] != float64(120) || got["output_tokens"] != float64(34) ||
			got["cache_read_tokens"] != float64(600) || got["cache_write_tokens"] != float64(40) ||
			got["reasoning_tokens"] != float64(12) || got["feature_id"] != "agent_tokens" {
	...
	err := trackTokens(context.Background(), TrackTokensRequest{
		CustomerID:       "team_123",
		FeatureID:        "agent_tokens",
