...
///
/// Tree structure we're dealing with:
/// ```
/// MaterialApp
///   └── Navigator
///         └── Overlay
///               └── _Theater
///                     ├── _OverlayEntryWidget (Route A - hidden)
///                     ├── _OverlayEntryWidget (ModalBarrier with BlockSemantics)
///                     └── _OverlayEntryWidget (Route B - current)
/// ```
///
/// BlockSemantics signals that everything painted before it should be blocked.
/// We detect it during traversal and clear collected siblings at the Overlay level.
class LayoutSnapshotCapture {
  static LayoutSnapshotCaptureResult? capture(
    ...
    try {
      ...
      final nodes = _traverse(rootElement, state);
      if (nodes.isEmpty) {
        return null;
      }
  ...
  static List<SnapshotNode> _traverse(
    Element element,
    _CaptureState state, {
    ...
    // Don't return early - continue traversing this subtree
    ...
    // IgnorePointer: skip entire subtree (not interactive)
    if (renderObject is RenderIgnorePointer && renderObject.ignoring) {
      return [];
    }
    ...
    // Create node if:
    // 1. This is the detected gesture element, OR
    // 2. Widget matches filter and is within screen bounds
    ...
    // Otherwise promote children (flatten)
    return children;
  }
  ...
  static List<SnapshotNode> _visitChildren(
    ...
    element.visitChildElements((child) {
      ...
      final childResults = _traverse(child, state, skipHitTest: skipHitTest);

      // Handle BlockSemantics flag
      // Only consume at Overlay level where routes are siblings
