...
public class MeasurePlugin: NSObject, FlutterPlugin {
    ...
    private func enableShakeDetector() throws {
        let shakeListener = ShakeListenerImpl { [weak self] in
            self?.channel?.invokeMethod(MethodConstants.callbackOnShakeDetected, arguments: nil)
        }
        Measure.setShakeListener(shakeListener)
    }

    private func disableShakeDetector() throws {
        Measure.setShakeListener(nil)
    }
}

private class ShakeListenerImpl: NSObject, MsrShakeListener {
    private let onShakeCallback: () -> Void

    init(onShakeCallback: @escaping () -> Void) {
        self.onShakeCallback = onShakeCallback
        super.init()
    }
