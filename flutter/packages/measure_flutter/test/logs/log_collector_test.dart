...
void main() {
  group('LogCollector', () {
    ...
    test('should not track logs when disabled', () {
      ...
      // When
      collector.trackLog(message, LogSeverity.info, attributes, timestamp);
    ...
    test('should track log with type log and userTriggered true', () {
      ...
      // When
      collector.trackLog(message, LogSeverity.info, attributes, null);
    ...
    test('should serialize log with severity_text and string keys', () {
      ...
      // When
      collector.trackLog(message, LogSeverity.warning, attributes, null);
    ...
    test('should pass through provided timestamp', () {
      // Given
      collector.register();
      final message = 'test log';
      final attributes = <String, AttributeValue>{};
      ...
      // Then
      expect(signalProcessor.trackedEvents.length, 1);
      expect(signalProcessor.trackedEvents[0].timestamp, timestamp);
    });
    ...
    test('should drop logs below the configured minimum level', () {
      ...
      // When
      collector.trackLog('debug', LogSeverity.debug, attributes, null);
      collector.trackLog('info', LogSeverity.info, attributes, null);
    ...
    test('should track logs at or above the configured minimum level', () {
      ...
      // When
      collector.trackLog('warning', LogSeverity.warning, attributes, null);
      collector.trackLog('error', LogSeverity.error, attributes, null);
      collector.trackLog('fatal', LogSeverity.fatal, attributes, null);

      // Then
      expect(signalProcessor.trackedLogs.length, 3);
    });
    ...
    test('should truncate message exceeding max length', () {
      ...
      collector.register();
      final message =
          List.filled(configProvider.maxLogMessageLength + 1, 'a').join();
      final attributes = <String, AttributeValue>{};
      ...
      expect(
        ...
        configProvider.maxLogMessageLength,
      );
    });
  });
}
