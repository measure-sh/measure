...
jest.mock("@/app/api/api_calls", () => {
  ...
  return {
    ...
    fetchRootSpanNamesFromServer: (...args: any[]) =>
      mockFetchRootSpanNamesFromServer(...args),
    saveListFiltersToServer: (...args: any[]) =>
      mockSaveListFiltersToServer(...args),
  };
});
...
describe("useFiltersStore", () => {
  beforeEach(() => {
    ...
    queryClient.clear();
    // The store hydrates onboarding state from localStorage on
    // construction, so any test that writes an onboarding entry
    // would leak into the next test's createFiltersStore() unless
    // we wipe it here.
  ...
  describe("setters", () => {
    ...
    it("setSelectedHttpMethods updates the list", () => {
      ...
      expect(store.getState().selectedHttpMethods).toEqual([
        HttpMethod.GET,
        HttpMethod.POST,
      ]);
    });
  ...
  describe("fetchApps", () => {
    ...
    it("picks app by initConfig.appId when no urlFilters.appId", async () => {
      ...
      await store
        .getState()
        .fetchApps("team-1", makeInitConfig({ appId: "app-b" }));
    ...
    it("team change: resets store and fetches new apps when teamId changes", async () => {
      const teamAApp = makeApp({
        id: "app-a",
        team_id: "team-1",
        name: "Team A App",
      });
      const teamBApp = makeApp({
        id: "app-b",
        team_id: "team-2",
        name: "Team B App",
      });
    ...
    it("team change: preserves config (per-page, not team-scoped)", async () => {
      const app = makeApp({ id: "app-a", team_id: "team-1" });
      mockFetchAppsFromServer.mockResolvedValueOnce({
        status: AppsApiStatus.Success,
        data: [app],
      ...
      // Switch team — fetchApps wipes everything team-scoped, but config
      // is per-page and must survive so computeFilters can keep
      // producing filters.ready against the same page context.
      mockFetchAppsFromServer.mockResolvedValueOnce({
        status: AppsApiStatus.NoApps,
        data: null,
      });
    ...
    it("team change: preserves onboarding state (per-app id, globally unique)", async () => {
      const app = makeApp({ id: "app-a", team_id: "team-1" });
      mockFetchAppsFromServer.mockResolvedValueOnce({
        status: AppsApiStatus.Success,
        data: [app],
      ...
      store.getState().setConfig(makeFilterConfig());
      await store.getState().fetchApps("team-1", makeInitConfig());
      store.getState().setOnboardingStep("app-a", "verify");
      ...
      expect(store.getState().onboarding["app-a"]).toEqual({
        step: "verify",
        platform: "Android",
    ...
    it("same team: skips refetch when appsApiStatus is already NoApps", async () => {
      // First fetch lands on NoApps. Subsequent fetchApps calls on the
      // same team (e.g. cross-page navigation) must short-circuit instead
      // of flipping the store back to Loading and re-hitting the server —
      // that's the visible "skeleton flash" on navigation in NoApps state.
      mockFetchAppsFromServer.mockResolvedValueOnce({
        status: AppsApiStatus.NoApps,
        data: null,
      });
      ...
      expect(mockFetchAppsFromServer).toHaveBeenCalledTimes(1);
      // Status never transitions through Loading on the short-circuit.
      expect(store.getState().appsApiStatus).toBe(AppsApiStatus.NoApps);
    });

    it("team change: refetches even when previous state was NoApps", async () => {
      // The NoApps short-circuit is scoped to same-team navigation; a
      // team change must wipe the store and refetch regardless.
      mockFetchAppsFromServer.mockResolvedValueOnce({
        status: AppsApiStatus.NoApps,
        data: null,
      });
      ...
      const teamBApp = makeApp({ id: "app-b", team_id: "team-2" });
      mockFetchAppsFromServer.mockResolvedValueOnce({
        status: AppsApiStatus.Success,
        data: [teamBApp],
      });
      ...
      expect(mockFetchAppsFromServer).toHaveBeenCalledTimes(2);
      expect(mockFetchAppsFromServer).toHaveBeenLastCalledWith("team-2");
      expect(store.getState().appsApiStatus).toBe(AppsApiStatus.Success);
    });
  ...
  describe("selectApp", () => {
    it("sets selectedApp and Loading before fetch resolves, then Success", async () => {
      ...
      mockFetchFiltersFromServer.mockReturnValueOnce(
        new Promise((r) => {
          resolveFetch = r;
        }) as any,
      );
    ...
    it("applies URL-based selections when urlFilters.appId === app.id", async () => {
      ...
      const state = store.getState();
      expect(state.selectedVersions.map((v) => v.name)).toEqual([
        "1.0.0",
        "1.0.2",
      ]);
      expect(state.selectedCountries).toEqual(["IN"]);
      expect(state.selectedSessionTypes).toEqual([
        SessionType.Crashes,
        SessionType.ANRs,
      ]);
    ...
    it("Spans filterSource: fetches root span names and caches them", async () => {
      store
        .getState()
        .setConfig(makeFilterConfig({ filterSource: FilterSource.Spans }));
      ...
      await store
        .getState()
        .selectApp(app, makeInitConfig({ filterSource: FilterSource.Spans }));
    ...
    it("applyFilterOptions: valid udAttrMatchers from URL are kept, invalid ones dropped", async () => {
      ...
      const validMatcher: UdAttrMatcher = {
        key: "user_id",
        type: "string",
        op: "eq",
        value: "x",
      };
      const unknownKey: UdAttrMatcher = {
        key: "not_a_key",
        type: "string",
        op: "eq",
        value: "x",
      };
      const unknownOp: UdAttrMatcher = {
        key: "user_id",
        type: "string",
        op: "invalid_op",
        value: "x",
      ...
      await store.getState().selectApp(
        ...
        makeInitConfig({
          urlFilters: {
            ...
            udAttrMatchers: [
              { key: "user_id", type: "string", op: "eq", value: "x" },
            ],
            freeText: "hello",
          },
        }),
  ...
  describe("computeFilters", () => {
    ...
    it("ready=true on NoData when showNoData=true", async () => {
      store
        .getState()
        .setConfig(
          makeFilterConfig({ showNoData: true, showNotOnboarded: false }),
        );
    ...
    it('ready=true on NoData when showNotOnboarded=true (the "showNotOnboarded-only" branch accepts NoData)', async () => {
      store
        .getState()
        .setConfig(
          makeFilterConfig({ showNoData: false, showNotOnboarded: true }),
        );
    ...
    it("ready=false on NotOnboarded when neither showNoData nor showNotOnboarded (strict mode still accepts it)", async () => {
      store
        .getState()
        .setConfig(
          makeFilterConfig({ showNoData: true, showNotOnboarded: true }),
        );
    ...
    it("neither showNoData nor showNotOnboarded: accepts Success, NoData, and NotOnboarded", async () => {
      store
        .getState()
        .setConfig(
          makeFilterConfig({ showNoData: false, showNotOnboarded: false }),
        );
  ...
  describe("filterShortCodePromise", () => {
    ...
    it("is recomputed when serialisedFilters changes and filters.ready becomes true", async () => {
      ...
      await expect(
        store.getState().filters.filterShortCodePromise,
      ).resolves.toBe("code-1");
    });
    ...
    it("does not re-call saveListFiltersToServer when selections are set to the same values", async () => {
      ...
      expect(mockSaveListFiltersToServer.mock.calls.length).toBe(
        callsAfterFetch,
      );
    });
    ...
    it("re-computes when body-relevant selections change", async () => {
      ...
      expect(mockSaveListFiltersToServer.mock.calls.length).toBeGreaterThan(
        callsAfterFetch,
      );
    });
    ...
    it("resets selections when switching to a different app", async () => {
      ...
      await store
        .getState()
        .fetchApps("team-1", makeInitConfig({ appId: "app-a" }));
      await flushPromises();
    ...
    it("does not refire shortFilters POSTs when navigating between two filterSources with different filter data", async () => {
      ...
      await store
        ...
        .fetchApps(
          "team-1",
          makeInitConfig({ filterSource: FilterSource.Events }),
        );
      await flushPromises();
      ...
      await store
        ...
        .fetchApps(
          "team-1",
          makeInitConfig({ filterSource: FilterSource.Anrs }),
        );
      await flushPromises();

      const callsAfterFirstVisits =
        mockSaveListFiltersToServer.mock.calls.length;
    ...
    it("ignores invalid URL filter values", async () => {
      ...
      expect(
        state.selectedCountries.every((c) => ["US", "UK"].includes(c)),
      ).toBe(true);
    });
    ...
    it("passes new udAttrMatchers to saveListFiltersToServer and updates filters.filterShortCodePromise", async () => {
      ...
      const latestCallArgs =
        mockSaveListFiltersToServer.mock.calls[callsBefore][0];
    ...
    it("fires a new POST and updates serialisedFilters when udAttrMatchers change", async () => {
      ...
      expect(store.getState().filters.serialisedFilters).not.toBe(
        serialisedBefore,
      );
  ...
  describe("filters.loading", () => {
    ...
    it("is true while apps are being fetched", async () => {
      ...
      mockFetchAppsFromServer.mockReturnValue(
        new Promise((r) => {
          resolveApps = r;
        }),
      );
    ...
    it("is true while filters are being fetched (apps succeeded)", async () => {
      ...
      mockFetchFiltersFromServer.mockReturnValue(
        new Promise((r) => {
          resolveFilters = r;
        }),
      );
  ...
  describe("onboarding state", () => {
    ...
    describe("hydration on construction", () => {
      ...
      it("seeds onboarding from valid localStorage entries", () => {
        window.localStorage.setItem(
          `${STORAGE_PREFIX}app-a`,
          JSON.stringify({
            step: "verify",
            platform: "iOS",
            flutterPlatform: "Android",
        ...
        const fresh = createFiltersStore();
        expect(fresh.getState().onboarding).toEqual({
          "app-a": {
            step: "verify",
            platform: "iOS",
            flutterPlatform: "Android",
      ...
      it("skips malformed JSON entries", () => {
        window.localStorage.setItem(`${STORAGE_PREFIX}bad`, "{not json");
        window.localStorage.setItem(
          `${STORAGE_PREFIX}good`,
          JSON.stringify({
            step: "integrate",
      ...
      it("skips entries with invalid step, platform, or flutterPlatform", () => {
        window.localStorage.setItem(
          `${STORAGE_PREFIX}wat`,
          JSON.stringify({
            step: "wat",
      ...
      it("skips entries missing the flutterPlatform field", () => {
        window.localStorage.setItem(
          `${STORAGE_PREFIX}legacy`,
          JSON.stringify({ step: "integrate", platform: "Android" }),
        );
        const fresh = createFiltersStore();
        expect(fresh.getState().onboarding).toEqual({});
      });

      it("ignores keys outside the onboarding prefix", () => {
        window.localStorage.setItem(
          "unrelated",
          JSON.stringify({
            step: "verify",
    ...
    describe("setOnboardingStep", () => {
      it("writes to in-memory state and storage", () => {
        ...
        expect(
          JSON.parse(window.localStorage.getItem(`${STORAGE_PREFIX}app-1`)!),
        ).toEqual({
          step: "verify",
          platform: "Android",
          flutterPlatform: "Android",
      ...
      it("preserves the existing platform and flutterPlatform when only step changes", () => {
      ...
      it("clears storage when step is verified (terminal)", () => {
        store.getState().setOnboardingStep("app-1", "verify");
        store.getState().setOnboardingStep("app-1", "verified");
        expect(
          window.localStorage.getItem(`${STORAGE_PREFIX}app-1`),
        ).toBeNull();
        expect(store.getState().onboarding["app-1"].step).toBe("verified");
      });

      it("clears storage when step transitions to create", () => {
    ...
    describe("setOnboardingPlatform", () => {
      ...
      it("does not write storage when current step is verified", () => {
        store.getState().setOnboardingStep("app-1", "verified");
        store.getState().setOnboardingPlatform("app-1", "iOS");
        expect(
          window.localStorage.getItem(`${STORAGE_PREFIX}app-1`),
    ...
    describe("setOnboardingFlutterPlatform", () => {
      ...
      it("preserves the existing step and platform when only flutterPlatform changes", () => {
        store.getState().setOnboardingStep("app-1", "verify");
        store.getState().setOnboardingPlatform("app-1", "Flutter");
        store.getState().setOnboardingFlutterPlatform("app-1", "iOS");
        expect(store.getState().onboarding["app-1"]).toEqual({
    ...
    describe("clearOnboarding", () => {
      it("removes the entry from in-memory state and storage", () => {
        store.getState().setOnboardingStep("app-1", "verify");
      ...
      it("does not affect other apps", () => {
        store.getState().setOnboardingStep("app-1", "verify");
        store.getState().setOnboardingStep("app-2", "integrate");
        store.getState().clearOnboarding("app-1");
        expect(store.getState().onboarding["app-2"]).toEqual({
          step: "integrate",
          platform: "Android",
          flutterPlatform: "Android",
        });
    ...
    describe("markAppOnboarded", () => {
      it("flips apps[].onboarded and selectedApp.onboarded to true atomically", () => {
        const app = makeApp({ id: "app-x", onboarded: false });
        store.setState({ apps: [app], selectedApp: app });
        store.getState().markAppOnboarded("app-x");
        expect(store.getState().apps[0].onboarded).toBe(true);
        expect(store.getState().selectedApp?.onboarded).toBe(true);
      });

      it("advances onboarding[appId].step to verified", () => {
        store.setState({ apps: [makeApp({ id: "app-x", onboarded: false })] });
        store.getState().setOnboardingStep("app-x", "verify");
        store.getState().markAppOnboarded("app-x");
        expect(store.getState().onboarding["app-x"].step).toBe("verified");
      });

      it("clears localStorage for the app", () => {
        store.setState({ apps: [makeApp({ id: "app-x", onboarded: false })] });
        store.getState().setOnboardingStep("app-x", "verify");
        expect(
          window.localStorage.getItem(`${STORAGE_PREFIX}app-x`),
        ).not.toBeNull();
        store.getState().markAppOnboarded("app-x");
        expect(
          window.localStorage.getItem(`${STORAGE_PREFIX}app-x`),
      ...
      it("leaves other apps untouched", () => {
        const a = makeApp({ id: "app-a", onboarded: false });
        const b = makeApp({ id: "app-b", onboarded: false });
        store.setState({ apps: [a, b], selectedApp: a });
        store.getState().markAppOnboarded("app-a");
        expect(
          store.getState().apps.find((x) => x.id === "app-a")?.onboarded,
        ).toBe(true);
        expect(
          store.getState().apps.find((x) => x.id === "app-b")?.onboarded,
        ).toBe(false);
      });

      it("preserves platform and flutterPlatform when advancing to verified", () => {
        store.setState({ apps: [makeApp({ id: "app-x", onboarded: false })] });
        store.getState().setOnboardingPlatform("app-x", "Flutter");
        store.getState().setOnboardingFlutterPlatform("app-x", "iOS");
        store.getState().setOnboardingStep("app-x", "verify");
        store.getState().markAppOnboarded("app-x");
        expect(store.getState().onboarding["app-x"]).toEqual({
          step: "verified",
          platform: "Flutter",
          flutterPlatform: "iOS",
        });
    ...
    describe("reset preserves onboarding", () => {
      // Onboarding is keyed by app id, not team — clearing filters
      // store on team change shouldn't drop wizard progress.
      it("keeps onboarding state across reset()", () => {
        store.getState().setOnboardingStep("app-1", "verify");
        store.getState().reset();
        expect(store.getState().onboarding["app-1"]).toEqual({
          step: "verify",
  ...
  // ====================================================================
  // SELECT APP — UNONBOARDED FAST PATH
  // ====================================================================
  describe("selectApp fast path for unonboarded apps", () => {
    it("sets NotOnboarded synchronously without firing fetchFiltersFromServer", async () => {
      const unonboarded = makeApp({ id: "unonboarded", onboarded: false });
      store.getState().setConfig(makeFilterConfig());

      await store.getState().selectApp(unonboarded, makeInitConfig());

      expect(store.getState().filtersApiStatus).toBe(
        FiltersApiStatus.NotOnboarded,
      );
      expect(store.getState().selectedApp?.id).toBe("unonboarded");
      expect(mockFetchFiltersFromServer).not.toHaveBeenCalled();
    });

    it("does not flash Loading on a same-app filterSource switch", async () => {
      const unonboarded = makeApp({ id: "u", onboarded: false });
      store.getState().setConfig(makeFilterConfig());
      // Initial select.
      await store.getState().selectApp(unonboarded, makeInitConfig());
      expect(store.getState().filtersApiStatus).toBe(
      ...
      // Switch filterSource — still the same app.
      await store
        .getState()
        .selectApp(
          unonboarded,
          makeInitConfig({ filterSource: FilterSource.Crashes }),
        );
      // Stays NotOnboarded throughout — no Loading transition.
      expect(store.getState().filtersApiStatus).toBe(
        FiltersApiStatus.NotOnboarded,
      );
      expect(mockFetchFiltersFromServer).not.toHaveBeenCalled();
    ...
    it("routes through the normal fetch path once markAppOnboarded flips the flag", async () => {
      const app = makeApp({ id: "tipping", onboarded: false });
      store.getState().setConfig(makeFilterConfig());
      await store.getState().selectApp(app, makeInitConfig());
      expect(mockFetchFiltersFromServer).not.toHaveBeenCalled();

      // Now the poller observes onboarded=true.
      store.setState({ apps: [app], selectedApp: app });
      store.getState().markAppOnboarded("tipping");

      mockFetchFiltersFromServer.mockResolvedValue({
        status: FiltersApiStatus.Success,
        data: makeFiltersData(),
      });
      // Simulate the next selectApp call that happens when Filters
      // re-mounts on /crashes after the user clicks View dashboard.
      const onboardedRef = store.getState().selectedApp!;
      await store
        .getState()
        .selectApp(
          onboardedRef,
