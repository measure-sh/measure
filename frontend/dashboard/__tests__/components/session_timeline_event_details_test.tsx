...
jest.mock("next/image", () => ({
  __esModule: true,
  default: ({ alt, src, onError, unoptimized, ...props }: any) => (
...
jest.mock("@/app/components/layout_snapshot", () => ({
  __esModule: true,
  default: ({ layoutUrl }: any) => (
    <div data-testid="layout-snapshot">{layoutUrl}</div>
  ),
}));
...
describe("SessionTimelineEventDetails", () => {
  ...
  describe("Details links", () => {
    ...
    it("renders ANR details link", () => {
      ...
      expect(link.closest("a")).toHaveAttribute(
        "href",
        "/team-1/errors/app-1/grp-2/ANR@Main.java",
      );
    });
    ...
    it("renders bug report details link", () => {
      ...
      expect(link.closest("a")).toHaveAttribute(
        "href",
        "/team-1/bug_reports/app-1/bug-123",
      );
    });
  ...
  describe("Demo mode details links", () => {
    ...
    it("renders non-clickable bug report label in demo mode", () => {
      ...
      expect(
        screen.getByText("View Bug Report Details").closest("a"),
      ).toBeNull();
    });
  });
      ...
      renderDetails({
        ...
        eventDetails: {
          ...
          attachments: [
            {
              key: "layout-1",
              location: "https://example.com/layout.json",
