...
import UserDefAttrSelector, {
  UdAttrMatcher,
} from "@/app/components/user_def_attr_selector";
...
describe("UserDefAttrSelector", () => {
  ...
  describe("Initial state from initialSelected", () => {
    it("pre-selects attrs from initialSelected", () => {
      ...
      // Only the attrs in initialSelected have a checked checkbox
      const checkboxes = screen.getAllByTestId(
        "checkbox",
      ) as HTMLInputElement[];
      expect(checkboxes.filter((c) => c.checked)).toHaveLength(1);
      expect(checkboxes.filter((c) => !c.checked)).toHaveLength(3);
    });
    ...
    it("uses op from initialSelected instead of default", () => {
      ...
      const userIdSelect = selects.find(
        (s) => (s as HTMLSelectElement).value === "contains",
      );
  ...
  describe("Operator selection", () => {
    ...
    it("fires onChangeSelected with updated op when changed", () => {
      ...
      renderSelector({
        ...
        initialSelected: [
          { key: "user_id", type: "string", op: "eq", value: "test" },
        ],
        onChangeSelected,
      });
  ...
  describe("Value input by type", () => {
    ...
    it("fires onChangeSelected with updated bool value", () => {
      ...
      renderSelector({
        ...
        initialSelected: [
          { key: "flag", type: "bool", op: "eq", value: false },
        ],
        onChangeSelected,
      });
    ...
    it("fires onChangeSelected with updated float64 value", () => {
      ...
      renderSelector({
        ...
        initialSelected: [
          { key: "ratio", type: "float64", op: "eq", value: 0 },
        ],
        onChangeSelected,
      });
