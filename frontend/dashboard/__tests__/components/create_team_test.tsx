...
function renderCreateTeam(
  overrides: Partial<{ onSuccess: (teamId: string) => void }> = {},
) {
  const props = {
    onSuccess: jest.fn(),
    ...overrides,
...
async function fillAndSubmit(name: string) {
  ...
  await act(async () => {
    fireEvent.submit(
      screen.getByPlaceholderText("Enter team name").closest("form")!,
    );
  });
}
  ...
  describe("Rendering", () => {
    ...
    it("disables submit button when team name is empty", () => {
      ...
      const submitButton = submitButtons.find(
        (b) => b.closest("button")?.getAttribute("type") === "submit",
      )!;
    ...
    it("shows error toast with error message", async () => {
      ...
      await waitFor(() => {
        expect(mockToastNegative).toHaveBeenCalledWith(
          "Error creating team: Name already taken",
        );
      });
    });
