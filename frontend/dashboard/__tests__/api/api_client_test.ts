...
if (
  ...
  !(
    globalThis.Request.prototype instanceof Object &&
    "url" in new (globalThis.Request as any)("http://x")
  )
) {
...
describe("ApiClient", () => {
  ...
  describe("redirectToLogin", () => {
    ...
    it("falls back to window.location.assign when no router is set", () => {
      client.redirectToLogin();
  ...
  describe("fetch", () => {
    ...
    it("propagates error when refresh fetch itself throws", async () => {
      ...
      await expect(client.fetch("/api/test")).rejects.toThrow(
        "Network failure",
      );
    });
  ...
  describe("posthog metrics", () => {
    ...
    it("normalizes UUIDs to :id in endpoint", async () => {
      ...
      await client.fetch(
        "/api/apps/5952c302-0e9f-4a00-98c4-f4e6106faedc/journey",
      );
    ...
    it("normalizes multiple UUIDs in endpoint", async () => {
      ...
      await client.fetch(
        "/api/teams/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee/apps/11111111-2222-3333-4444-555555555555/filters",
      );
