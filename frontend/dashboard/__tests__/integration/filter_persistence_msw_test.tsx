...
async function expectPersistence<T>(
    ...
    await renderAndWait()
    await act(async () => { setFilter() })
    expect(getFilter()).toEqual(expectedValue)
    await unmountAndRemount()
    expect(getFilter()).toEqual(expectedValue)
}

const { AppVersion, OsVersion, SessionType, BugReportStatus, SpanStatus, HttpMethod } = require('@/app/api/api_calls')
...
describe('Per-source filter persistence', () => {
    ...
    it('session types persist', async () => {
        await expectPersistence(
            () => filtersStore.getState().setSelectedSessionTypes([SessionType.Crashes, SessionType.ANRs]),
    ...
    it('http methods persist', async () => {
        await expectPersistence(
            () => filtersStore.getState().setSelectedHttpMethods([HttpMethod.GET, HttpMethod.POST]),
    ...
    it('root span name persists', async () => {
        await expectPersistence(
            () => filtersStore.getState().setSelectedRootSpanName('checkout_full_display'),
...
describe('Combined filter persistence', () => {
    it('multiple filters changed simultaneously all persist', async () => {
        ...
        expect(filtersStore.getState().selectedDeviceManufacturers).toEqual(['Google'])
