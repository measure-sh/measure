...
jest.mock("@nivo/bar", () => ({
  ...
  ResponsiveBar: ({ data }: any) => (
    <div data-testid="nivo-bar-chart">{data?.length ?? 0} groups</div>
  ),
}));
...
describe.each([
  ...
  async function renderAndWaitForData() {
    renderWithProviders(
      <ExceptionsOverview exceptionsType={exceptionsType} teamId="test-team" />,
    );
    await waitFor(
      () => {
  ...
  describe("page load", () => {
    ...
    it("renders exception type and message as subtitle", async () => {
      await renderAndWaitForData();
      expect(
        screen.getByText(/NullPointerException.*null object reference/),
      ).toBeTruthy();
    });
      ...
      server.use(
        http.get(`*/api/apps/:appId/${apiPath}`, ({ request }) => {
          ...
          if (
            ...
            url.pathname.match(new RegExp(`${apiPath}/[^/]+/`))
          )
            return;
  ...
  describe("filters", () => {
    ...
    it("network generation change sends network_generations in POST", async () => {
      ...
      shortFilterBodies.length = 0;
      await act(async () => {
        filtersStore.getState().setSelectedNetworkGenerations(["4g"]);
      });
      await waitFor(() => expect(shortFilterBodies.length).toBeGreaterThan(0), {
        timeout: 5000,
      });
    ...
    it("locale change sends locales in POST", async () => {
      ...
      shortFilterBodies.length = 0;
      await act(async () => {
        filtersStore.getState().setSelectedLocales(["de-DE"]);
      });
      await waitFor(() => expect(shortFilterBodies.length).toBeGreaterThan(0), {
        timeout: 5000,
      });
    ...
    it("udAttr matcher sends ud_expression in POST", async () => {
      ...
      const expr = JSON.parse(
        shortFilterBodies[shortFilterBodies.length - 1].filters.ud_expression,
      );
  ...
  describe("URL sync", () => {
    ...
    it("table row links include crash/anr group id and timestamp params", async () => {
      ...
      expect(href).toContain(
        `/test-team/${exceptionsType === ExceptionsType.Crash ? "crashes" : "anrs"}/`,
      );
...
describe.each([
  ...
  async function renderAndWaitForData() {
    ...
    await waitFor(
      () => {
        expect(
          screen.getAllByText("View Session Timeline").length,
        ).toBeGreaterThan(0);
      },
      { timeout: 10000 },
    );
  ...
  describe("error states", () => {
    it("shows error when instances API returns 500", async () => {
      server.use(
        http.get(
          `*/api/apps/:appId/${apiPath}/:groupId/${exceptionsType === ExceptionsType.Crash ? "crashes" : "anrs"}`,
          () => {
            return new HttpResponse(null, { status: 500 });
          },
        ),
    ...
    it("shows plot error when detail plot returns 500", async () => {
      server.use(
        http.get(
          `*/api/apps/:appId/${apiPath}/:groupId/plots/instances`,
          () => {
            return new HttpResponse(null, { status: 500 });
          },
        ),
    ...
    it("shows No Data when distribution plot returns empty", async () => {
      server.use(
        http.get(
          `*/api/apps/:appId/${apiPath}/:groupId/plots/distribution`,
          () => {
  ...
  describe("instance pagination", () => {
    ...
    it("Previous from instance 2 goes back to instance 1", async () => {
      ...
      const instanceVariant =
        exceptionsType === ExceptionsType.Crash ? "crash" : "anr";

      server.use(
        http.get(
          `*/api/apps/:appId/${apiPath}/:groupId/${instanceEndpoint}`,
          ({ request }) => {
            const url = new URL(request.url);
            const offset = url.searchParams.get("offset");
            if (offset === "1") {
              const page2 = makeExceptionInstanceFixture({
                variant: instanceVariant as any,
              });
            ...
            return HttpResponse.json(
              makeExceptionInstanceFixture({ variant: instanceVariant as any }),
            );
          },
        ),
      );
    ...
    it("instance with attachments renders images", async () => {
      ...
      server.use(
        http.get(
          `*/api/apps/:appId/${apiPath}/:groupId/${instanceEndpoint}`,
          () => {
            const fixture = makeExceptionInstanceFixture({
              variant: instanceVariant as any,
            });
