...
describe("extractTocEntries", () => {
  it("skips the first h1 as the page title", () => {
    const content = ["# Page Title", "## Section One", "## Section Two"].join(
      "\n",
    );
  ...
  it("ignores h4 and deeper headings", () => {
    const content = ["# Title", "## Section", "#### Deep Heading"].join("\n");
  ...
  it("deduplicates across different heading levels", () => {
    const content = ["# Title", "## Setup", "### Setup"].join("\n");
  ...
  it("strips emphasis markers from heading text", () => {
    const content = ["# Title", "## **Bold** and *italic* and `code`"].join(
      "\n",
    );
  ...
  it("generates slug-style IDs from heading text", () => {
    ...
    // "&" is dropped and the spaces around it each become a hyphen, matching
    // github-slugger / rehype-slug (the rendered heading id is double-hyphen).
  ...
  it("preserves snake_case in heading text and id (matches github-slugger)", () => {
    const content = ["# Title", "### `list_apps`", "### `get_filters`"].join(
      "\n",
    );
    ...
    expect(entries).toEqual([
      { id: "list_apps", text: "list_apps", level: 3 },
      { id: "get_filters", text: "get_filters", level: 3 },
    ]);
  });
});
...
describe("cleanContent", () => {
  it("strips single-line HTML comments", () => {
    expect(cleanContent("Hello <!-- omit in toc --> World")).toBe(
      "Hello  World",
    );
  });
...
describe("extractDescription", () => {
  ...
  it("returns empty string when no plain paragraph is present", () => {
    const content = ["# Title", "", "* only a list", "* of bullet items"].join(
      "\n",
    );
...
describe("stripSearchContent", () => {
  ...
  it("strips image syntax", () => {
    expect(stripSearchContent("See ![diagram](img.png) below")).toBe(
      "See below",
    );
  });
  ...
  it("unwraps markdown links to visible text", () => {
    expect(
      stripSearchContent("Read [the docs](https://example.com) now."),
