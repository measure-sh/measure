...
describe('GitHub docs links have been replaced', () => {
  for (const file of FILES_WITH_REWRITTEN_LINKS) {
    it(`${file} does not contain GitHub docs URLs`, () => {
      ...
      expect(content).not.toContain('github.com/measure-sh/measure/blob/main/docs/')
      expect(content).not.toContain('tab=readme-ov-file#docs')
    })
  }

  it('page.tsx links to /docs/hosting for self-hosting', () => {
  ...
  it('pricing/page.tsx links to /docs/hosting for self-hosting', () => {
    const content = fs.readFileSync(path.join(APP_DIR, 'pricing/page.tsx'), 'utf-8')

    expect(content).toContain('href="/docs/hosting"')
  })

  it('why-measure/page.tsx links to /docs/hosting for self-hosting', () => {
    const content = fs.readFileSync(path.join(APP_DIR, 'why-measure/page.tsx'), 'utf-8')

    expect(content).toContain('href="/docs/hosting"')
  })

  it('sdk_configurator.tsx links to /docs/features/configuration-options', () => {
    const content = fs.readFileSync(path.join(APP_DIR, 'components/sdk_configurator.tsx'), 'utf-8')
  ...
  it('user_journeys.tsx links to /docs/features/configuration-options#journey-sampling', () => {
    const content = fs.readFileSync(path.join(APP_DIR, 'components/user_journeys.tsx'), 'utf-8')

    expect(content).toContain('href="/docs/features/configuration-options#journey-sampling"')
  })

  it('filters.tsx links to /docs', () => {
    const content = fs.readFileSync(path.join(APP_DIR, 'components/filters.tsx'), 'utf-8')

    expect(content).toContain('href="/docs"')
  })

  it('apps/page.tsx links to /docs', () => {
    const content = fs.readFileSync(path.join(APP_DIR, '[teamId]/apps/page.tsx'), 'utf-8')
  ...
  it('session_timelines/page.tsx links to /docs/features/feature-session-timelines', () => {
    ...
    expect(content).toContain('href="/docs/features/feature-session-timelines"')
  })
})
...
describe('rewritten links do not use target=_blank', () => {
  it('sdk_configurator.tsx does not open docs in new tab', () => {
    ...
    const docsLinkLine = lines.find(l => l.includes('/docs/features/configuration-options'))
  ...
  it('session_timelines/page.tsx does not open docs in new tab', () => {
    ...
    const lines = content.split('\n')
    const docsLinkLine = lines.find(l => l.includes('/docs/features/feature-session-timelines'))
