...
describe("stripHtmlComments", () => {
  it("removes a single comment", () => {
    expect(stripHtmlComments("hello <!-- comment --> world")).toBe(
      "hello  world",
    );
  });

  it("removes multiple comments", () => {
    expect(stripHtmlComments("a <!-- one --> b <!-- two --> c")).toBe(
      "a  b  c",
    );
  });
...
describe("mdPathToSlug", () => {
  ...
  it("converts a nested .md file", () => {
    expect(mdPathToSlug("features/feature-crash-reporting.md")).toBe(
  ...
  it("converts deeply nested README.md", () => {
    expect(mdPathToSlug("sdk-upgrade-guides/android/README.md")).toBe(
      "/docs/sdk-upgrade-guides/android",
    );
  });

  it("strips fragment identifiers", () => {
    expect(mdPathToSlug("features/feature-crash-reporting.md#setup")).toBe(
      "/docs/features/feature-crash-reporting",
    );
  });
...
describe("getTitleFromFile", () => {
  ...
  it("returns h1 title from a markdown file", () => {
    ...
    expect(
      getTitleFromFile("/docs", "features/feature-crash-reporting.md"),
    ).toBe("Crash Reporting");
...
describe("generateDocsNav", () => {
        ...
        children: [
          {
            ...
            slug: "/docs/features/feature-session-timelines",
          },
          {
            title: "Crash Reporting",
            slug: "/docs/features/feature-crash-reporting",
          },
        ],
      },
    ]);
  ...
  it("handles indented bullet children for nested groups", () => {
    fs.readFileSync.mockImplementation((p) => {
      ...
      if (p.includes("bug-report-android"))
        return "# Bug Report Android\nContent";
    ...
    expect(nav[0].children).toEqual([
      {
        ...
        children: [
          {
            title: "Bug Report Android",
            slug: "/docs/features/feature-bug-report-android",
          },
          {
            title: "Bug Report iOS",
            slug: "/docs/features/feature-bug-report-ios",
          },
        ],
      },
    ]);
  ...
  it("handles inline links for non-feature sections", () => {
    fs.readFileSync.mockImplementation((p) => {
      ...
      if (p.includes("configuration-options"))
    ...
    expect(nav).toEqual([
      {
        ...
        slug: "/docs/features/configuration-options",
      },
    ]);
  });
  ...
  it("appends Further Reading links after body sections", () => {
    fs.readFileSync.mockImplementation((p) => {
      ...
      if (p.includes("sdk-integration-guide"))
    ...
    expect(nav[0]).toEqual({
      ...
      slug: "/docs/sdk-integration-guide",
    });
  ...
  it("does not process the same inline section twice", () => {
    fs.readFileSync.mockImplementation((p) => {
      ...
      if (p.includes("performance-impact"))
    ...
    expect(nav).toEqual([
      {
        ...
        slug: "/docs/features/performance-impact",
      },
    ]);
  });
  ...
  it("returns empty array for README with no sections or links", () => {
    fs.readFileSync.mockReturnValue(
      "Just some text with no links or headings.",
    );
    fs.existsSync.mockReturnValue(true);
