import DocsSearch, {
  getMatchSnippet,
  highlightTerms,
} from "@/app/docs/components/docs_search";
...
describe("highlightTerms", () => {
  ...
  it("wraps matched term in mark element", () => {
    const { container } = render(
      <>{highlightTerms("the crash report", "crash")}</>,
    );
  ...
  it("highlights are case-insensitive", () => {
    const { container } = render(
      <>{highlightTerms("Crash Reporting", "crash")}</>,
    );
    const mark = container.querySelector("mark");
  ...
  it("highlights multiple occurrences of a term", () => {
    const { container } = render(
      <>{highlightTerms("crash and crash again", "crash")}</>,
    );
  ...
  it("highlights multiple different terms", () => {
    const { container } = render(
      <>{highlightTerms("crash and memory issues", "crash memory")}</>,
    );
    const marks = container.querySelectorAll("mark");
  ...
  it("preserves non-matching text around highlights", () => {
    const { container } = render(
      <>{highlightTerms("before crash after", "crash")}</>,
    );
  ...
  it("handles regex special characters in query safely", () => {
    const { container } = render(
      <>{highlightTerms("price is $100 (USD)", "$100")}</>,
    );
    const mark = container.querySelector("mark");
...
describe("DocsSearch component", () => {
  ...
  it("returns null when not open", () => {
    const { container } = render(
      <DocsSearch open={false} onOpenChange={jest.fn()} />,
    );
  ...
  it("shows placeholder text before typing", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Type to search documentation..."),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows snippet with highlighted match", async () => {
    mockFetchWith(mockEntries);
    const { container } = render(
      <DocsSearch open={true} onOpenChange={jest.fn()} />,
    );
  ...
  it("closes on backdrop click", async () => {
    ...
    const onOpenChange = jest.fn();
    const { container } = render(
      <DocsSearch open={true} onOpenChange={onOpenChange} />,
    );
    await act(async () => {});
