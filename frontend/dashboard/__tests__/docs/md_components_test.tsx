import {
  ...
  rewriteHref,
  rewriteImgSrc,
} from "@/app/docs/components/md_components";
...
describe("rewriteHref", () => {
  describe("passthrough cases", () => {
    ...
    it("returns anchor links unchanged", () => {
      expect(
        rewriteHref("#section", ["features", "feature-crash-reporting"]),
      ).toBe("#section");
    });
    ...
    it("returns https URLs unchanged", () => {
      expect(rewriteHref("https://example.com", [])).toBe(
        "https://example.com",
      );
    });

    it("returns mailto links unchanged", () => {
      expect(rewriteHref("mailto:test@example.com", [])).toBe(
  ...
  describe("relative .md links", () => {
    it("rewrites a sibling .md file from root", () => {
      expect(rewriteHref("faqs.md", ["sdk-integration-guide"])).toBe(
        "/docs/faqs",
      );
    });
    ...
    it("rewrites a sibling .md file from a subdirectory", () => {
      expect(
        rewriteHref("feature-crash-reporting.md", [
          "features",
          "feature-session-timelines",
        ]),
      ).toBe("/docs/features/feature-crash-reporting");
    });

    it("rewrites a relative path with directory", () => {
      expect(
        rewriteHref("features/feature-crash-reporting.md", [
    ...
    it("rewrites a parent directory reference with ../", () => {
      expect(
        rewriteHref("../faqs.md", ["features", "feature-crash-reporting"]),
      ).toBe("/docs/faqs");
    });

    it("rewrites README.md to directory index route", () => {
      expect(
        rewriteHref("README.md", ["features", "feature-crash-reporting"]),
      ).toBe("/docs/features");
    });

    it("rewrites subdirectory README.md to directory route", () => {
      expect(rewriteHref("hosting/README.md", ["sdk-integration-guide"])).toBe(
        "/docs/hosting",
      );
    });
    ...
    it("preserves anchor fragments after rewriting", () => {
      expect(
        rewriteHref("configuration-options.md#journey-sampling", [
          "features",
          "feature-crash-reporting",
        ]),
      ).toBe("/docs/features/configuration-options#journey-sampling");
    });
  ...
  describe("links that escape the docs tree", () => {
    it("generates a GitHub URL when .md link traverses above docs root", () => {
      const result = rewriteHref("../../backend/README.md", [
        "sdk-integration-guide",
      ]);
    ...
    it("generates a GitHub URL for non-.md links that escape docs tree", () => {
      const result = rewriteHref(
        "../self-host/session-data/config.toml.example",
        ["CONTRIBUTING"],
      );

      expect(result).toBe(
        "https://github.com/measure-sh/measure/blob/main/self-host/session-data/config.toml.example",
      );
    });
  });
  ...
  describe("README/index page links (isIndex=true)", () => {
    it("resolves a sibling .md link relative to the directory itself", () => {
      ...
      expect(
        rewriteHref("android-v0.16.0.md", ["sdk-upgrade-guides"], true),
      ).toBe("/docs/sdk-upgrade-guides/android-v0.16.0");
    });
...
describe("rewriteImgSrc", () => {
  describe("passthrough cases", () => {
    ...
    it("returns http URLs unchanged", () => {
      expect(rewriteImgSrc("http://example.com/img.png")).toBe(
        "http://example.com/img.png",
      );
    });

    it("returns https URLs unchanged", () => {
      expect(rewriteImgSrc("https://example.com/img.png")).toBe(
  ...
  describe("relative asset paths", () => {
    it("rewrites assets/ prefix", () => {
      expect(rewriteImgSrc("assets/screenshot.png")).toBe(
    ...
    it("rewrites ../assets/ prefix", () => {
      expect(rewriteImgSrc("../assets/create-app.png")).toBe(
        "/docs/assets/create-app.png",
      );
    });

    it("rewrites deeply nested assets/ path", () => {
      expect(rewriteImgSrc("some/deep/path/assets/image.png")).toBe(
        "/docs/assets/image.png",
      );
    });

    it("preserves the filename and subdirectories after assets/", () => {
      expect(rewriteImgSrc("assets/screenshots/dark/crash.png")).toBe(
        "/docs/assets/screenshots/dark/crash.png",
      );
    });
  });
...
describe("createMarkdownComponents", () => {
  ...
  describe("link component", () => {
    it("renders internal .md links as internal navigation", () => {
      ...
      const LinkComponent = components.a as React.FC<{
        href?: string;
        children?: React.ReactNode;
      }>;

      render(
        <LinkComponent href="feature-session-timelines.md">
          Session Timelines
        </LinkComponent>,
      );
  ...
  describe("image component", () => {
    it("rewrites relative asset paths", () => {
      ...
      const ImgComponent = components.img as React.FC<{
        src?: string;
        alt?: string;
      }>;
  ...
  describe("heading components", () => {
    it("renders h2 with anchor link", () => {
      const components = createMarkdownComponents([]);
      const H2Component = components.h2 as React.FC<{
        children?: React.ReactNode;
        id?: string;
      }>;
    ...
    it("renders h3 with anchor link", () => {
      const components = createMarkdownComponents([]);
      const H3Component = components.h3 as React.FC<{
        children?: React.ReactNode;
        id?: string;
      }>;
  ...
  describe("blockquote component", () => {
    it("renders a regular blockquote", () => {
      ...
      const { container } = render(
        <BlockquoteComponent>Regular quote</BlockquoteComponent>,
      );
  ...
  describe("code component", () => {
    it("renders inline code without className", () => {
      const components = createMarkdownComponents([]);
      const CodeComponent = components.code as React.FC<{
        children?: React.ReactNode;
        className?: string;
      }>;
    ...
    it("renders code block with language className", () => {
      ...
      const { container } = render(
        <CodeComponent className="language-typescript">
          const x = 1
        </CodeComponent>,
      );
