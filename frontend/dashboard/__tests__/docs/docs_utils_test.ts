...
beforeAll(() => {
  ...
  fs.writeFileSync(
    path.join(tmpDir, "features", "crash.md"),
    "# Crash Reporting\n\nCrash content.",
  );
...
describe("docs.ts filesystem functions", () => {
  ...
  beforeAll(() => {
    ...
    jest
      ...
      .mockImplementation((p: fs.PathOrFileDescriptor, options?: any) => {
        const pStr = p.toString();
        if (pStr.startsWith(contentDocsPath)) {
          const relative = pStr.slice(contentDocsPath.length);
    ...
    jest
      ...
      .mockImplementation((p: fs.PathLike, options?: any) => {
        const pStr = p.toString();
        if (pStr.startsWith(contentDocsPath)) {
          const relative = pStr.slice(contentDocsPath.length);
  ...
  describe("getAllDocSlugs", () => {
    ...
    it("includes direct .md files as single-element slugs", () => {
      const slugs = getAllDocSlugs();
      const guideSlug = slugs.find(
        (s: string[]) => s.length === 1 && s[0] === "guide",
      );
    ...
    it("includes directory README as directory slug", () => {
      const slugs = getAllDocSlugs();
      const hostingSlug = slugs.find(
        (s: string[]) => s.length === 1 && s[0] === "hosting",
      );
    ...
    it("includes nested .md files", () => {
      const slugs = getAllDocSlugs();
      const dockerSlug = slugs.find(
        (s: string[]) => JSON.stringify(s) === '["hosting","docker"]',
      );
