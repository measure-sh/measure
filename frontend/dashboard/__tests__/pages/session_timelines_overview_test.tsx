...
jest.mock("@/app/query/hooks", () => ({
  ...
  useSessionTimelinesOverviewQuery: () =>
    mockUseSessionTimelinesOverviewQuery(),
...
jest.mock("@/app/components/session_timelines_overview_plot", () => () => (
  <div data-testid="session-timelines-overview-plot-mock">
    SessionTimelinesOverviewPlot Rendered
  </div>
));
...
describe("SessionTimelinesOverview Component", () => {
  beforeEach(() => {
    ...
    mockUseSessionTimelinesOverviewQuery.mockReturnValue({
      data: undefined,
      status: "pending" as string,
      isFetching: true,
      error: null,
  ...
  it("renders main sessions UI, updates URL when filters become ready, and renders table headers", async () => {
    ...
    expect(
      await screen.findByTestId("session-timelines-overview-plot-mock"),
    ).toBeInTheDocument();
  ...
  it("renders appropriate link for each session that includes teamId, app_id and session_id", async () => {
    ...
    expect(link).toHaveAttribute(
      "href",
      "/123/session_timelines/app1/session1",
    );
