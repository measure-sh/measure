...
const { GET } = require('@/app/auth/callback/google/route')

function makeRequest(params: string) {
    return { url: `http://localhost:3000/auth/callback/google${params}` } as unknown as Request
}
...
describe('Google Callback Route', () => {
    ...
    it('dashboard flow: exchanges code via backend and sets cookies', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                method: 'POST',
                body: JSON.stringify({ type: 'code', state: 'dashboard-state', code: 'google-code' }),
            })
        )
