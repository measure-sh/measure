...
describe("NetworkTrends - Demo mode", () => {
  ...
  it("renders demo endpoint data correctly", () => {
    ...
    expect(
      screen.getByText(/payments\.demo-provider\.com\/\*\/checkout/),
    ).toBeInTheDocument();
  });
...
describe("NetworkTrends - Non-demo mode", () => {
  ...
  it("renders title and learn more subtitle", async () => {
    ...
    expect(screen.getByText("Top Endpoints")).toBeInTheDocument();
    // The "Learn more" doc link now lives in an info tooltip beside the title.
    expect(
      screen
  ...
  it("renders table with data after successful fetch", async () => {
    ...
    expect(screen.getByText("Endpoint")).toBeInTheDocument();
    expect(
      screen.getByText(/api\.example\.com\/v1\/slow$/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/api\.example\.com\/v1\/slower/),
    ).toBeInTheDocument();
  });
  ...
  it("switches to error rate tab and shows correct data", async () => {
    ...
    expect(
      screen.getByText(/api\.example\.com\/v1\/errors/),
    ).toBeInTheDocument();
  ...
  it("switches to frequency tab and shows correct data", async () => {
    ...
    expect(
      screen.getByText(/cdn\.example\.com\/images\/\*/),
    ).toBeInTheDocument();
  });
