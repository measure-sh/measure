...
jest.mock("@/app/query/hooks", () => ({
  ...
  useNetworkEndpointStatusCodesQuery: () =>
    mockUseNetworkEndpointStatusCodesQuery(),
...
describe("NetworkDetails", () => {
  ...
  it("updates URL with domain and path when filters become ready", async () => {
    ...
    mockUseNetworkEndpointStatusCodesQuery.mockReturnValue({
      data: {
        status_codes: [200],
        data_points: [
          { datetime: "2024-01-01", total_count: 100, count_200: 100 },
        ],
      },
      status: "success",
      error: null as Error | null,
  ...
  it("shows latency error message when latency API fails", async () => {
    ...
    mockUseNetworkEndpointTimelineQuery.mockReturnValue({
      data: {
        interval: 5,
        points: [{ elapsed: 1, domain: "a", path_pattern: "/b", count: 1 }],
      },
      status: "success",
      error: null as Error | null,
    });
  ...
  it("shows status distribution error message when API fails", async () => {
    ...
    expect(
      screen.getByText(
        "Error fetching status distribution data, please change filters & try again",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("shows timeline error message when timeline API fails", async () => {
    ...
    expect(
      screen.getByText(
        "Error fetching timeline data, please change filters & try again",
      ),
    ).toBeInTheDocument();
  });
