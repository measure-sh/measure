...
describe("computeEventBreakdown", () => {
  it("computes session starts as dailyUsers * averageAppOpens", () => {
    const events = computeEventBreakdown(
      inputsWith({ dailyUsers: 500, averageAppOpens: 4 }),
    );
...
describe("calculate", () => {
  ...
  describe("raw cost is $1/GB", () => {
    it("rawMonthlyCost equals totalGBPerMonth times PRICE_PER_GB_MONTH", () => {
      ...
      expect(result.rawMonthlyCost).toBeCloseTo(
        result.totalGBPerMonth * PRICE_PER_GB_MONTH,
      );
    });
    ...
    it("rawMonthlyCost is not clamped to minimum", () => {
      ...
      if (!result.isFreeTier) {
        expect(result.rawMonthlyCost).toBe(
          result.totalGBPerMonth * PRICE_PER_GB_MONTH,
        );
      }
    });
  ...
  describe("single event type isolation", () => {
    ...
    it("enabling launch sampling adds bytes", () => {
      ...
      const withLaunch = computeBytesPerDay(
        computeEventBreakdown({ ...zeroInputs, launchSamplePercent: 1 }),
      );
    ...
    it("enabling error rate adds crash bytes (larger) and timeline bytes", () => {
      const base = computeBytesPerDay(computeEventBreakdown(zeroInputs));
      const withErrors = computeBytesPerDay(
        computeEventBreakdown({ ...zeroInputs, errorRatePercent: 1 }),
      );
    ...
    it("enabling perf spans adds bytes", () => {
      const base = computeBytesPerDay(computeEventBreakdown(zeroInputs));
      const withPerf = computeBytesPerDay(
        computeEventBreakdown({
          ...zeroInputs,
          perfSpanSamplePercent: 1,
          perfSpanCount: 10,
        }),
      );
    ...
    it("enabling journey sampling adds bytes", () => {
      const base = computeBytesPerDay(computeEventBreakdown(zeroInputs));
      const withJourney = computeBytesPerDay(
        computeEventBreakdown({ ...zeroInputs, journeySamplePercent: 1 }),
      );
      expect(withJourney).toBeGreaterThan(base);
    });

    it("perf spans scale with perfSpanCount", () => {
      const with5 = computeBytesPerDay(
        computeEventBreakdown({
          ...zeroInputs,
          perfSpanSamplePercent: 1,
          perfSpanCount: 5,
      ...
      const with10 = computeBytesPerDay(
        computeEventBreakdown({
          ...zeroInputs,
          perfSpanSamplePercent: 1,
          perfSpanCount: 10,
