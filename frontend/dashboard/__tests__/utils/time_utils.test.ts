...
describe("formatMillisToHumanReadable", () => {
  ...
  it("should return seconds for values between 1 second and 1 minute", () => {
    ...
    expect(formatMillisToHumanReadable(59999)).toBe("59.999s");
  });

  it("should return minutes for values between 1 minute and 1 hour", () => {
    expect(formatMillisToHumanReadable(60000)).toBe("1m");
    expect(formatMillisToHumanReadable(120000)).toBe("2m");
    expect(formatMillisToHumanReadable(3599999)).toBe("59m 59.999s");
  });
  ...
  it("should return hours for values between 1 hour and 1 day", () => {
    ...
    expect(formatMillisToHumanReadable(86399999)).toBe("23h 59m 59.999s");
  });
  ...
  it("should return days for values greater than 1 day", () => {
    ...
    expect(formatMillisToHumanReadable(604799999)).toBe("6d 23h 59m 59.999s");
  });
...
describe("formatChartFormatTimestampToHumanReadable", () => {
  ...
  it("should throw on invalid timestamps", () => {
    const timestamp = "invalid-timestamp";
    expect(() =>
      formatChartFormatTimestampToHumanReadable(timestamp),
    ).toThrow();
  });
});
...
describe("plot time group utils", () => {
  it("maps date range to plot time groups", () => {
    ...
    expect(
      getPlotTimeGroupForRange("2026-02-01T00:00:00Z", "2026-02-06T00:00:00Z"),
    ).toBe("hours");
    expect(
      getPlotTimeGroupForRange("2026-01-01T00:00:00Z", "2026-03-15T00:00:00Z"),
    ).toBe("days");
    expect(
      getPlotTimeGroupForRange("2025-01-01T00:00:00Z", "2026-01-01T00:00:00Z"),
    ).toBe("months");
  });
  ...
  it("uses days fallback for invalid or non-increasing ranges", () => {
    ...
    expect(getPlotTimeGroupForRange("2026-02-01T06:00:00Z", "invalid")).toBe(
      "days",
    );
    expect(
      getPlotTimeGroupForRange("2026-02-01T06:00:00Z", "2026-02-01T06:00:00Z"),
    ).toBe("days");
    expect(
      getPlotTimeGroupForRange("2026-02-01T07:00:00Z", "2026-02-01T06:00:00Z"),
    ).toBe("days");
  });

  it("handles exact threshold boundaries", () => {
    expect(
      getPlotTimeGroupForRange("2026-02-01T00:00:00Z", "2026-02-02T00:00:00Z"),
    ).toBe("minutes");
    expect(
      getPlotTimeGroupForRange("2026-02-01T00:00:00Z", "2026-02-01T12:00:00Z"),
    ).toBe("minutes");
    expect(
    ...
    expect(
      getPlotTimeGroupForRange("2026-01-01T00:00:00Z", "2026-04-02T00:00:00Z"),
    ).toBe("months");
  });

  it("returns expected nivo config for each time group", () => {
    expect(getPlotTimeGroupNivoConfig("minutes").xScalePrecision).toBe(
      "minute",
    );
  ...
  it("formats tooltip dates for minutes and hours groups", () => {
    expect(formatPlotTooltipDate("2026-02-01T10:00:00", "minutes")).toBe(
      "1 Feb, 2026, 10:00 AM",
    );
    expect(formatPlotTooltipDate("2026-02-01T10:00:00", "hours")).toBe(
      "1 Feb, 2026, 10:00 AM",
    );
  });
  ...
  it("falls back to days format for unknown time group", () => {
    ...
    expect(formatPlotTooltipDate("2026-02-01", "unknown" as any)).toBe(
      "1 Feb, 2026",
    );
  });
});
