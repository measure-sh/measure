/**
 * Pins the fumadocs CodeBlock internals that the blog TrackedCodeBlock
 * depends on, so a fumadocs upgrade that changes them fails here instead
 * of silently skewing analytics:
 *
...
jest.mock("next/navigation", () => ({
  __esModule: true,
  usePathname: () => "/blog/mobile-breaks-differently",
}));

import { track } from "@/app/utils/analytics/track";
import { TrackedCodeBlock } from "@/app/blog/components/tracked_code_block";
...
describe("TrackedCodeBlock", () => {
  ...
  it("tracks a copy when the copy button is clicked via its inner icon", async () => {
    ...
    expect(track).toHaveBeenCalledWith("blog_action_click", {
      action: "copy_code",
      blog_section: "mobile-breaks-differently",
    });
  });
