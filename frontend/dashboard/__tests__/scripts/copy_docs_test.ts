...
describe("getTitleFromFile", () => {
  it("reads h1 title from a file", () => {
    expect(getTitleFromFile(tmpDir, "sdk-integration-guide.md")).toBe(
      "SDK Integration Guide",
    );
  });
...
describe("parseDirectoryNav", () => {
  ...
  it("reads titles from linked files", () => {
    const children = parseDirectoryNav(tmpDir, "hosting");
    const selfHosting = children.find(
      (c: any) => c.slug === "/docs/hosting/self-hosting",
    );
...
describe("generateDocsNav", () => {
  ...
  it("includes crash reporting as a feature child", () => {
    const nav = generateDocsNav(tmpDir);
    const features = nav.find((item: any) => item.title === "Features");
    const crash = features.children.find(
      (c: any) => c.slug === "/docs/features/feature-crash-reporting",
    );
  ...
  it("creates nested Bug Reporting group with platform children", () => {
    const nav = generateDocsNav(tmpDir);
    const features = nav.find((item: any) => item.title === "Features");
    const bugReporting = features.children.find(
      (c: any) => c.title === "Bug Reporting",
    );
  ...
  it("includes inline-linked sections (Configuration Options)", () => {
    const nav = generateDocsNav(tmpDir);
    const config = nav.find(
      (item: any) => item.slug === "/docs/features/configuration-options",
    );
...
describe("parseDirectoryNav edge cases", () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      path.join(edgeDir, "mydir", "page-a.md"),
      "# Page A\n\nContent A.",
    );
    // An extra unlinked file
    fs.writeFileSync(
      path.join(edgeDir, "mydir", "extra.md"),
      "# Extra\n\nExtra content.",
    );
    ...
    fs.writeFileSync(
      path.join(edgeDir, "mydir", "subdir", "child.md"),
      "# Child Page\n\nChild content.",
    );
    ...
    fs.writeFileSync(
      path.join(edgeDir, "mydir", "unseen-dir", "README.md"),
      "# Unseen Dir\n\nContent.",
    );
    fs.writeFileSync(
      path.join(edgeDir, "mydir", "unseen-dir", "page.md"),
      "# Unseen Page\n\nContent.",
    );
  });
  ...
  it("deduplicates links with the same slug", () => {
    ...
    const pageASlugs = children.filter(
      (c: any) => c.slug === "/docs/mydir/page-a",
    );
  ...
  it("creates nested children for subdirectory README links", () => {
    ...
    const childPage = subDir.children.find(
      (c: any) => c.slug === "/docs/mydir/subdir/child",
    );
