    ...
    const nav = [
      {
        ...
        children: [
          {
            title: "L2",
            children: [{ title: "L3", slug: "/docs/deep" }],
          },
        ],
      },
    ];
...
describe("generateLlmsTxt", () => {
  ...
  it("produces H2 sections for items with children", () => {
    ...
    expect(result).toContain(
      "- [Crash Reporting](https://measure.sh/docs/features/crash)",
    );
    expect(result).toContain(
      "- [ANR Reporting](https://measure.sh/docs/features/anr)",
    );
  });
  ...
  it("flattens nested grandchildren into the parent H2 section", () => {
    ...
    expect(result).toContain("## Features");
    expect(result).toContain(
      "- [Android](https://measure.sh/docs/features/bug-android)",
    );
    expect(result).toContain(
      "- [iOS](https://measure.sh/docs/features/bug-ios)",
    );
  });
  ...
  it("ends with ## Optional section linking to llms-full.txt", () => {
    ...
    expect(result).toContain(
      "- [llms-full.txt](https://measure.sh/llms-full.txt)",
    );
  });
...
describe("rewriteRelativeLinks", () => {
  it("rewrites relative .md links to absolute URLs", () => {
    const input =
      "See [Crash Reporting](features/feature-crash-reporting.md) for details.";
  ...
  it("rewrites .md links with anchors", () => {
    const input =
      "See [Config](features/configuration-options.md#sdk-options) for details.";
    const result = rewriteRelativeLinks(input);
  ...
  it("rewrites relative image paths", () => {
    ...
    expect(result).toBe(
      "![screenshot](https://measure.sh/docs/assets/pic.png)",
    );
  });
  ...
  it("handles README.md links", () => {
    ...
    expect(result).toBe(
      "See [Hosting](https://measure.sh/docs/hosting) for details.",
    );
  });
...
describe("generateLlmsFullTxt", () => {
  ...
  it("deduplicates slugs", () => {
    ...
    const result = generateLlmsFullTxt(tmpDir, nav);
    const guideMatches =
      result.match(/Source: https:\/\/measure\.sh\/docs\/guide$/gm) || [];
