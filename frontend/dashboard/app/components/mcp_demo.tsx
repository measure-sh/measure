...
const USER_PROMPT =
  "Can you help me fix crash d58064f1-80d9-4a6a-9f0f-1af51ccfcb19?";
  ...
  useEffect(() => {
    ...
    switch (phase) {
      ...
      case Phase.ToolCallAppear:
        timeout = setTimeout(
          () => setPhase(Phase.ToolResultAppear),
          TOOL_CALL_FADE,
        );
        break;
    ...
    while (remaining.length > 0) {
      ...
      if (codeBlockMatch) {
        parts.push(
          <div key={key++} className="my-2 rounded bg-muted/50 px-3 py-2">
            <code className="text-foreground">{codeBlockMatch[2]}</code>
          </div>,
        );
      ...
      if (inlineCodeMatch) {
        parts.push(
          <code
            key={key++}
            className="rounded bg-muted/50 px-1 py-0.5 text-red-700 dark:text-red-400"
          >
            {inlineCodeMatch[1]}
          </code>,
        );
    ...
    <div className="w-full font-display px-8 md:px-0">
      ...
      >
        ...
        >
          ...
          {showToolCall && (
            <div
              className={`mt-4 rounded border-l-2 border-amber-400 bg-muted/50 p-3 transition-opacity duration-300 ${phase >= Phase.ToolCallAppear ? "opacity-100" : "opacity-0"}`}
            >
          ...
          {showToolResult && (
            <div
              className={`mt-3 rounded border-l-2 border-sky-400 bg-muted/50 p-3 transition-opacity duration-300 ${phase >= Phase.ToolResultAppear ? "opacity-100" : "opacity-0"}`}
            >
