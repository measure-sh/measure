...
const NetworkErrorRatePlot: React.FC<NetworkErrorRatePlotProps> = ({ data }) => {
  ...
  return (
    <div className="flex font-body items-center justify-center w-full h-[36rem]">
      ...
      <ResponsiveLine
        ...
        enableSlices="x"
        sliceTooltip={({ slice }) => {
          const pointData = slice.points[0]?.data as unknown as { x: string; xFormatted: string; y: number; error_count: number; total_count: number }
          return (
            <div className="bg-accent text-accent-foreground flex flex-col p-2 text-xs rounded-md">
              ...
              {slice.points.map((point) => (
                <div className="flex flex-row items-center p-2" key={point.id}>
                  ...
                  <div className="px-2" />
                  <p>{point.serieId}: {Number(point.data.y).toFixed(2)}%</p>
                </div>
              ))}
              <p className='p-2'>Total Requests: {pointData?.total_count?.toLocaleString()}</p>
              <p className='px-2 pb-2'>Errors: {pointData?.error_count?.toLocaleString()}</p>
            </div>
          )
        }}
      />
...
export default NetworkErrorRatePlot
