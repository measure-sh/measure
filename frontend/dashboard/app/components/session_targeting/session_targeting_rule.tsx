...
const getEventTypesFromResponse = (response: typeof sessionTargetingConfigResponse) => {
    return response.result?.events?.map(event => event.type) || [];
};

const getEventAttributes = (response: typeof sessionTargetingConfigResponse, eventType: string) => {
    const event = response.result?.events?.find(e => e.type === eventType);
    return event?.attrs || [];
};

const getSessionAttributes = (response: typeof sessionTargetingConfigResponse) => {
    return response.result?.session_attrs || [];
};

const doesEventSupportUdAttrs = (response: typeof sessionTargetingConfigResponse, eventType: string): boolean => {
    const event = response.result?.events?.find(e => e.type === eventType);
    return event?.ud_attrs === true;
};

const getUserDefinedAttributes = (response: typeof sessionTargetingConfigResponse) => {
    return response.result?.event_ud_attrs?.key_types || [];
};

const getOperatorTypesMapping = (response: typeof sessionTargetingConfigResponse) => {
...
const getAvailableAttributes = (
    SessionTargetingConfig: typeof sessionTargetingConfigResponse,
...
export default function SessionTargetingRule({ params, isEditMode }: SessionTargetingRuleProps) {
    ...
    const setupCreateMode = (config: typeof sessionTargetingConfigResponse) => {
    ...
    const updateEventCondition = (conditionIndex: number, type: string) => {
        const updatedConditions = ruleState.eventConditions.conditions.map((condition, index) =>
    ...
    const addAttribute = (conditionIndex: number, attributeType: AttributeType) => {
        const condition = ruleState.eventConditions.conditions[conditionIndex]
        ...
        const updatedConditions = ruleState.eventConditions.conditions.map((cond, index) =>
            index === conditionIndex
                ? {
                    ...cond,
    ...
    const updateSessionAttribute = (
        ...
        const condition = ruleState.sessionConditions.conditions[conditionIndex]
        const currentAttrs = condition?.attrs
        if (!currentAttrs) return
        ...
        const updatedConditions = ruleState.sessionConditions.conditions.map((cond, index) =>
            index === conditionIndex
    ...
    if (pageState.status === PageStatus.Loading) {
        return (
            <div className="flex flex-col selection:bg-yellow-200/75 items-start">
                <LoadingSpinner />
            </div>
                    ...
                    {pageState.status === PageStatus.Ready && (
                        <>
                            <div className="grid gap-y-8 items-center max-w-2xl" style={{ gridTemplateColumns: '120px 1fr' }}>
                                ...
                                <div className="flex items-center">
                                    <SwitchToggle
                                        toggled={ruleState.status === 'enabled'}
