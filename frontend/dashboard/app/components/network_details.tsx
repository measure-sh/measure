...
import {
  useNetworkEndpointLatencyQuery,
  useNetworkEndpointStatusCodesQuery,
  useNetworkEndpointTimelineQuery,
...
export default function NetworkDetails({ params }: NetworkDetailsProps) {
  ...
  const statusDistributionQuery = useNetworkEndpointStatusCodesQuery(
    domain,
    path,
  );
  ...
  const latencyStatus =
    latencyQuery.status === "success" && latencyQuery.data === null
      ? ("nodata" as const)
      : latencyQuery.status;
  const statusDistributionStatus =
    statusDistributionQuery.status === "success" &&
    statusDistributionQuery.data === null
      ? ("nodata" as const)
      : statusDistributionQuery.status;
  const timelineStatus =
    timelineQuery.status === "success" && timelineQuery.data === null
      ? ("nodata" as const)
      : timelineQuery.status;

  const plotTimeGroup: PlotTimeGroup = getPlotTimeGroupForRange(
    filters.startDate,
    filters.endDate,
  );
  const shouldRenderLatencyPlot =
    latencyStatus === "success" && latencyQuery.data !== null;
  const shouldRenderStatusDistributionPlot =
    statusDistributionStatus === "success" &&
    statusDistributionQuery.data !== null;
  const shouldRenderTimelinePlot =
    timelineStatus === "success" &&
    timelineQuery.data !== null &&
    timelineQuery.data.points.length > 0;
  ...
  return (
    <div className="flex flex-col items-start w-full">
          ...
          <div className="flex font-body items-center justify-center w-full h-[36rem]">
            ...
            {latencyStatus === "error" && (
              <p className="font-body text-sm">
                Error fetching latency data, please change filters & try again
              </p>
            )}
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="flex font-body items-center justify-center w-full h-[36rem]">
            ...
            {statusDistributionStatus === "error" && (
              <p className="font-body text-sm">
                Error fetching status distribution data, please change filters &
                try again
              </p>
            )}
            ...
            {shouldRenderStatusDistributionPlot && (
              <NetworkEndpointStatusCodesPlot
                statusCodes={statusDistributionQuery.data!.status_codes}
                data={statusDistributionQuery.data!.data_points}
                plotTimeGroup={plotTimeGroup}
              />
            )}
          </div>
      ...
      {filters.ready && timelineStatus !== "nodata" && (
        <div className="flex flex-col w-full">
          <div className="py-6" />
          ...
          {!shouldRenderTimelinePlot && (
            <div className="flex font-body items-center justify-center w-full h-[36rem]">
              ...
              {timelineStatus === "error" && (
                <p className="font-body text-sm">
                  Error fetching timeline data, please change filters & try
                  again
                </p>
              )}
            </div>
