...
const TraceViz: React.FC<TraceVizProps> = ({ inputTrace }) => {
  ...
  return (
    <div className="flex flex-col w-full h-[600px] relative font-body text-black rounded-md overflow-hidden">

      {/* Panel */}
      <div
        className={`absolute overflow-auto z-50 top-0 left-0 h-full w-64 md:w-72 lg:w-80 bg-neutral-800 p-4 text-white text-xs break-words transform transition-transform duration-300 ease-in-out ${selectedSpan !== undefined ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
      ...
      <div className="flex flex-row w-full h-full items-start pt-4 pb-8 overflow-x-hidden overflow-y-auto">
        ...
        <div className='flex flex-col w-1/4 min-w-[200px] max-w-[300px] pl-2 mt-12 overflow-x-auto select-none shrink-0'>
          {trace.spans.filter((span) => span.visibility !== SpanVisibility.Hidden).map((span, index) => (
            <button key={span.span_id} className={`flex flex-row items-center h-12 w-fit min-w-full focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] ${index % 2 === 0 ? 'bg-zinc-50' : ''} ${span.numberOfChildren! > 0 ? 'hover:bg-zinc-100' : ''}`}
        ...
        <div className="flex flex-col w-full select-none">
          ...
          {trace.spans.filter((span) => span.visibility !== SpanVisibility.Hidden).map((span, spanIndex) => (
            <div key={span.span_id} className={`relative w-full h-12 ${spanIndex % 2 === 0 ? 'bg-zinc-50' : ''}`}>
              {/* duration label */}
              <p
              ...
              {/* span bar */}
              <button
                className={`absolute top-6 h-3 min-w-[8px] focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] ${bgSpanColorMap.get(span.span_id)} ${span === selectedSpan ? "bg-neutral-800 hover:bg-neutral-950" : ""} select-none`}
                style={{
              ...
              >
                {span.checkpoints?.map((checkpoint) => (
                  <button
                    ...
                    className={`absolute w-0.5 h-2 rounded-full mt-0.5 focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] ${bgCheckPointColorMap.get(span.span_id)} ${checkpoint === selectedCheckpoint ? "bg-neutral-800 hover:bg-neutral-950" : ""}`}
