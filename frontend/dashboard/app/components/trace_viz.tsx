...
const TraceViz: React.FC<TraceVizProps> = ({ inputTrace }) => {
  ...
  const assignColors = (span: Span, colorIndex: number) => {
    bgSpanColorMap.set(
      span.span_id,
      bgSpanColors[colorIndex % bgSpanColors.length],
    );
    bgCheckPointColorMap.set(
      span.span_id,
      bgCheckPointColors[colorIndex % bgCheckPointColors.length],
    );
    bgLineColorMap.set(
      span.span_id,
      bgLineColors[colorIndex % bgLineColors.length],
    );
    borderColorMap.set(
      span.span_id,
  ...
  const addSpanAndChildren = (span: Span, depth: number) => {
    ...
    span.leftPercent =
      (spanStart.diff(traceStartTime).milliseconds / inputTrace.duration) *
        100 +
      marginPercent;
    span.widthPercent = Math.max(
      (span.duration / inputTrace.duration) * 100 - marginPercent * 2,
      1,
    ); // min 1%
    ...
    span.checkpoints?.forEach((c) => {
      ...
      const checkpointPercent =
        (checkPointTime.diff(traceStartTime).milliseconds /
          inputTrace.duration) *
          100 +
        marginPercent;
      c.leftPercent =
        ((checkpointPercent - span.leftPercent!) / span.widthPercent!) * 100;
    });
  ...
  const toggleSpanVisibility = (targetSpanId: string) => {
    ...
    const updatedTargetSpan = {
      ...targetSpan,
      visibility: newTargetVisibility,
    };
  ...
  return (
    <div className="flex flex-col w-full min-h-[400px] max-h-[600px] relative font-body text-foreground rounded-md overflow-hidden">
      {/* Panel */}
      <div
        className={`absolute overflow-auto z-50 top-0 left-0 h-full w-64 md:w-72 lg:w-80 bg-accent p-4 text-accent-foreground text-xs break-words transform transition-transform duration-300 ease-in-out ${
          selectedSpan !== undefined ? "translate-x-0" : "-translate-x-full"
        }`}
      >
          ...
          <div>
            ...
            <div className="flex flex-row mt-2">
              ...
              <p className={valueStyle}>
                {" "}
                {formatDateToHumanReadableDateTime(selectedSpan.start_time)}
              </p>
            </div>
            <div className="flex flex-row mt-2">
              ...
              <p className={valueStyle}>
                {" "}
                {formatDateToHumanReadableDateTime(selectedSpan.end_time)}
              </p>
            </div>
            <div className="flex flex-row mt-2">
              ...
              <p className={valueStyle}>
                {" "}
                {formatMillisToHumanReadable(selectedSpan.duration)}
              </p>
            </div>
            <div className="flex flex-row mt-2">
              ...
              <p className={valueStyle}>
                {" "}
                {selectedSpan.parent_id ? selectedSpan.parent_id : "--"}
              </p>
            </div>
            <div className="flex flex-row mt-2">
            ...
            <div className="flex flex-row mt-2">
              ...
              <p
                className={`${valueStyle} ${selectedSpan.status === 1 ? "text-green-500" : selectedSpan.status === 2 ? "text-red-500" : ""}`}
              >
                {" "}
                {selectedSpan.status === 0
                  ? "Unset"
                  : selectedSpan.status === 1
                    ? "Okay"
                    : "Error"}
              </p>
            </div>
            {selectedSpan.user_defined_attributes !== null &&
              selectedSpan.user_defined_attributes !== undefined &&
              Object.entries(selectedSpan.user_defined_attributes!).map(
                ([attrKey, attrValue]) => (
                  <div className="flex flex-row mt-2" key={attrKey}>
            ...
            <div className="flex flex-row mt-2">
              ...
              <p className={`${valueStyle}`}>
                {selectedSpan.checkpoints !== null &&
                selectedSpan.checkpoints.length > 0
                  ? ": "
                  : ": []"}
              </p>
            </div>
            {selectedSpan.checkpoints?.map((checkpoint, _) => (
              <button
                className={`flex flex-col group mt-1 py-2 px-2 w-full rounded-md focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] ${selectedCheckpoint === checkpoint ? "bg-background" : "hover:bg-background"}`}
                key={checkpoint.name}
                onClick={() => setSelectedCheckpoint(checkpoint)}
              >
                <div className="flex flex-row mt-2">
                  <p
                    className={`${keyStyle} ${selectedCheckpoint === checkpoint ? "text-foreground dark:text-accent-foreground/60" : "group-hover:text-foreground text-accent-foreground/60 dark:group-hover:text-accent-foreground/60"}`}
                  >
                    Name
                  </p>
                  <p
                    className={`${valueStyle} ${selectedCheckpoint === checkpoint ? "text-foreground" : "group-hover:text-foreground text-accent-foreground"}`}
                  >
                    {" "}
                    {checkpoint.name}
                  </p>
                ...
                <div className="flex flex-row mt-2">
                  ...
                  >
                    {" "}
                    {formatDateToHumanReadableDateTime(checkpoint.timestamp)}
                  </p>
                </div>
              </button>
            ))}
      ...
      <div className="flex flex-row w-full h-full items-start pt-4 pb-8 bg-background text-foreground overflow-x-hidden overflow-y-auto">
        ...
        <div className="flex flex-col w-1/4 min-w-[200px] max-w-[300px] pl-2 mt-12 overflow-x-auto select-none shrink-0">
          {trace.spans
            .filter((span) => span.visibility !== SpanVisibility.Hidden)
            .map((span, index) => (
              <button
                key={span.span_id}
                className={`flex flex-row items-center h-12 w-fit min-w-full focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] ${index % 2 === 0 ? "bg-secondary" : ""} ${span.numberOfChildren! > 0 ? (index % 2 === 0 ? "hover:bg-secondary/50" : "hover:bg-accent") : ""}`}
              ...
              >
                ...
                {Array.from({ length: span.depth || 0 }).map((_, index) => (
                  <div
                    ...
                    className={`h-full w-[0.5px] ml-3.5 ${bgLineColorMap.get(span.span_id)}`}
                  />
                ))}
                {/* horizontal connecting line */}
                <div
                  className={`h-[0.5px] w-2.5 ${bgLineColorMap.get(span.span_id)}`}
                />

                {/* connecting circle container */}
                <div className="flex flex-col items-center">
                  {/* connecting circle vertical invisible line for positioning */}
                  {span.numberOfChildren! > 0 && (
                    <div className={`h-5 w-[0.5px] invisible`} />
                  )}

                  {/* connecting circle */}
                  <div
                    className={`h-2 w-2 rounded-full ${bgLineColorMap.get(span.span_id)}`}
                  />

                  {/* connecting circle vertical visible line */}
                  {span.numberOfChildren! > 0 && (
                    <div
                      className={`h-5 w-[0.5px] ${bgLineColorMap.get(span.span_id)}`}
                    />
                  )}
                </div>
                <p
                  className={`text-[10px] border py-0.1 px-1 ml-1 rounded-[2px] ${borderColorMap.get(span.span_id)}`}
                >
                  {span.numberOfChildren!}{" "}
                </p>
                <p
                  className={`text-xs mb-1 text-nowrap ml-2 truncate max-w-[150px]`}
                >
                  {span.span_name}
                </p>
                <p className={`text-[8px] ml-2 mr-2 text-muted-foreground`}>
                  {span.visibility === SpanVisibility.Expanded &&
                  span.numberOfChildren! > 0
                    ? "\u02c5"
                    : span.visibility === SpanVisibility.Collapsed &&
                        span.numberOfChildren! > 0
                      ? "\u02c3"
                      : ""}
                </p>
        ...
        <div className="flex flex-col w-full select-none">
          ...
          <div className="w-full flex flex-row justify-between px-2 pt-2">
            <p className="text-[10px] truncate">
              {formatMillisToHumanReadable(0)}
            </p>
            <p className="text-[10px] truncate">
              {formatMillisToHumanReadable(trace.duration * 0.2)}
            </p>
            <p className="text-[10px] truncate">
              {formatMillisToHumanReadable(trace.duration * 0.4)}
            </p>
            <p className="text-[10px] truncate">
              {formatMillisToHumanReadable(trace.duration * 0.6)}
            </p>
            <p className="text-[10px] truncate">
              {formatMillisToHumanReadable(trace.duration * 0.8)}
            </p>
            <p className="text-[10px] truncate">
              {formatMillisToHumanReadable(trace.duration)}
            </p>
          </div>
          ...
          {trace.spans
            ...
            .map((span, spanIndex) => (
              <div
                key={span.span_id}
                className={`relative w-full h-12 ${spanIndex % 2 === 0 ? "bg-secondary" : ""}`}
              >
                {/* duration label */}
                <p
