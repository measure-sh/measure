    ...
    return (
        <div className="w-full p-4 font-display">
            <div className="w-full mx-auto">
                ...
                {/* Dashboard Section */}
                <div className="bg-white border border-neutral-400 rounded-md p-4 mb-4 shadow-lg">
                    <div className="flex items-center gap-2 mb-4">
                        <Settings className="w-4 h-4 mb-1 text-black" />
                        <p className="text-black font-display">Dashboard Configuration</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4">
                        {(Object.entries(configState) as [keyof ConfigState, DataConfig][]).map(([key, config]) => {
                            ...
                            return (
                                <div
                                    key={key}
                                    className={`bg-white ${active ? 'border-yellow-400' : 'bg-white border-gray-200 opacity-70'} border-2 rounded-md p-4`}
                                >
                ...
                <div className="flex justify-center mb-4">
                    <div className="relative">
                        ...
                        <div className="flex flex-col items-center">
                            <div className="w-px h-6 bg-gray-200 mb-2"></div>
                            <div className={`w-2 h-2 ${syncing ? 'bg-yellow-300 animate-pulse' : 'bg-gray-200'} rounded-full`}></div>
                ...
                {/* Server Section */}
                <div className="bg-white border border-neutral-400 rounded-md p-4 md:p-6 mb-4 shadow-lg">
                    <div className="flex items-center gap-2 mb-4">
                        <Server className="w-4 h-4 mb-1 text-black" />
                        <p className="text-black font-display">Measure Server</p>
                    </div>

                    <div className="bg-slate-50 rounded-md p-4">
                        <div className="text-sm space-y-2">
                            <div className="flex justify-between items-center">
                                <span className="text-black">Config Status:</span>
                                <span className={`${syncing ? 'text-yellow-500' : ''}`}>
                ...
                <div className="bg-white border border-neutral-400 rounded-md p-4 md:p-6 shadow-lg">
                    <div className="flex items-center gap-2 mb-4">
                        <Smartphone className="w-4 h-4 mb-1 text-black" />
                        <p className="text-black font-display">Mobile Devices</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                        {devices.map((device) => (
                            <div
                                key={device.id}
                                className={`bg-neutral-800 border-2 rounded-md p-3`}
                            >
                                <div className="flex flex-col items-start justify-between gap-2">
                                    <div className="flex flex-row items-center gap-2">
                                        <Smartphone className="w-3 h-3 mb-1 text-white" />
                                        <p className="text-sm text-white">{device.name}</p>
                                    </div>
                                    <div className="flex items-center gap-2.5 px-1">
                                        <div className={`w-1.5 h-1.5 rounded-full ${syncing ? 'bg-yellow-500 animate-pulse' : 'bg-gray-200'}`}></div>
                                        <span className={`text-xs text-gray-200 ${syncing ? 'text-yellow-500' : ''}`}>
                                            {syncing ? 'Syncing' : 'Synced'}
                                        </span>
                                    </div>
                                </div>
