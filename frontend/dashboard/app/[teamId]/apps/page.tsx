...
export default function Apps({ params }: { params: { teamId: string } }) {
  ...
  const { data: thresholdPrefs, status: thresholdPrefsStatus } =
    useAppThresholdPrefsQuery(appId);
  ...
  const canChangeAppThresholdPrefs =
    authzAndMembers?.can_change_app_threshold_prefs === true;
  ...
  // UI-only local state
  const [
    appRetentionPeriodConfirmationDialogOpen,
    setAppRetentionPeriodConfirmationDialogOpen,
  ] = useState(false);
  const [appNameConfirmationDialogOpen, setAppNameConfirmationDialogOpen] =
    useState(false);
  const [appApiKeyConfirmationDialogOpen, setAppApiKeyConfirmationDialogOpen] =
    useState(false);
  const [saveAppNameButtonDisabled, setSaveAppNameButtonDisabled] =
    useState(true);
  const [appName, setAppName] = useState("");

  // Editable form values
  const [updatedRetention, setUpdatedRetention] = useState<
    typeof emptyAppRetention | null
  >(null);
  const [editableThresholdPrefs, setEditableThresholdPrefs] = useState<
    typeof defaultAppThresholdPrefs | null
  >(null);

  // Use editable values, falling back to server data
  const currentRetention =
    updatedRetention ?? appRetention ?? emptyAppRetention;
  const currentThresholdPrefs =
  ...
  const handleRetentionPeriodChange = (newRetentionPeriod: string) => {
    setUpdatedRetention({
      retention: displayTextToRetentionPeriodMap.get(newRetentionPeriod)!,
    });
  };

  const handleUpdateAppThresholdPrefs = async () => {
    if (
      currentThresholdPrefs.error_good_threshold <=
      currentThresholdPrefs.error_caution_threshold
    ) {
      toastNegative(
    ...
    if (
      currentThresholdPrefs.error_good_threshold <= 0 ||
      currentThresholdPrefs.error_good_threshold > 100
    ) {
      toastNegative(
        "Error updating thresholds",
        "Good threshold must be between 0 and 100",
      );
      return;
    }
    if (
      currentThresholdPrefs.error_caution_threshold < 0 ||
      currentThresholdPrefs.error_caution_threshold >= 100
    ) {
      toastNegative(
        "Error updating thresholds",
        "Caution threshold must be between 0 and 100",
      );
      return;
    }
    ...
    if (
      currentThresholdPrefs.error_spike_min_rate_threshold <= 0 ||
      currentThresholdPrefs.error_spike_min_rate_threshold > 100
    ) {
      toastNegative(
        "Error updating thresholds",
        "Spike threshold must be between 0 (exclusive) and 100",
      );
      return;
    }
  ...
  const appThresholdValuesChanged =
    currentThresholdPrefs.error_good_threshold !==
      savedThresholdPrefs.error_good_threshold ||
    currentThresholdPrefs.error_caution_threshold !==
      savedThresholdPrefs.error_caution_threshold ||
    currentThresholdPrefs.error_spike_min_count_threshold !==
      savedThresholdPrefs.error_spike_min_count_threshold ||
    currentThresholdPrefs.error_spike_min_rate_threshold !==
      savedThresholdPrefs.error_spike_min_rate_threshold;
      ...
      {pageDataSuccess && (
        <div className="w-full max-w-6xl">
          ...
          <DangerConfirmationDialog
            ...
            open={appApiKeyConfirmationDialogOpen}
            affirmativeText="Yes, rotate key"
            cancelText="Cancel"
            onAffirmativeAction={() => {
          ...
          <DangerConfirmationDialog
            body={
              <p className="font-body">
                ...
                ? <br /> <br /> This change only affects new sessions, current
                sessions will retain their original retention period.
              </p>
            }
            open={appRetentionPeriodConfirmationDialogOpen}
            affirmativeText="Yes, I'm sure"
            cancelText="Cancel"
            onAffirmativeAction={() => {
          ...
          <div className="font-body">
            ...
            {thresholdPrefsStatus === "error" && (
              <p className="font-body text-sm">
                Error fetching app threshold preferences, please refresh page to
                try again
              </p>
            )}
              ...
              <div className="flex flex-col items-start mt-6 gap-3 w-full">
                <div className="flex flex-row items-center gap-2">
                  ...
                  <SdkConfigNumericInput
                    ...
                    fixedWidth={14}
                    disabled={
                      !canChangeAppThresholdPrefs ||
                      updateThresholdPrefsMutation.isPending
                    }
                    onChange={(value) => {
              ...
              <>
                ...
                <div className="flex flex-row items-center mt-2">
                  <DropdownSelect
                    ...
                    initialSelected={
                      retentionPeriodToDisplayTextMap.get(
                        (appRetention ?? emptyAppRetention).retention!,
                      )!
                    }
                    onChangeSelected={(item) =>
                      handleRetentionPeriodChange(item as string)
                    }
                  />
                  <Button
                    variant="outline"
                    ...
                    onClick={() =>
                      setAppRetentionPeriodConfirmationDialogOpen(true)
                    }
                  >
                    Save
                  </Button>
            ...
            <div className="flex flex-row items-center mt-2">
              ...
              <Button
                ...
                disabled={
                  !canRenameApp ||
                  saveAppNameButtonDisabled ||
                  changeAppNameMutation.isPending
                }
                className="m-4"
