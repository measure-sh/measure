      ...
      {bugReportApiStatus === BugReportApiStatus.Success &&
        <div>
          ...
          <div className="flex flex-row">
            ...
            <div className="px-2" />
            <button onClick={updateBugReportStatus} disabled={updateBugReportStatusApiStatus === UpdateBugReportStatusApiStatus.Loading} className={`w-fit outline-none hover:enabled:bg-yellow-200 focus-visible:enabled:bg-yellow-200 active:enabled:bg-yellow-300 font-display border border-black rounded-md transition-colors duration-100 py-2 px-4 ${(updateBugReportStatusApiStatus === UpdateBugReportStatusApiStatus.Loading) ? 'pointer-events-none' : 'pointer-events-auto'}`}>{bugReport.status === 0 ? "Close" : "Re-Open"}</button>
          </div>
