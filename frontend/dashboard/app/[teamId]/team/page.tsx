...
import { useSessionStore } from '@/app/stores/provider'
...
export default function TeamOverview({ params }: { params: { teamId: string } }) {
  ...
  const { data: authzAndMembers, status: authzAndMembersStatus } = useAuthzAndMembersQuery(params.teamId)
  const { data: pendingInvites, status: pendingInvitesStatus } = usePendingInvitesQuery(params.teamId)
  const { data: teamSlackConnectUrl, status: slackConnectUrlStatus } = useTeamSlackConnectUrlQuery(
    currentUserId,
    params.teamId,
    `${process.env.NEXT_PUBLIC_SITE_URL}/auth/callback/slack`
  )
  const { data: teamSlack, status: slackStatusQueryStatus } = useTeamSlackStatusQuery(params.teamId)

  // Type for member entries in authz response
  type AuthzMember = { id: string; email: string; role: string; name: string | null; last_sign_in_at: string; created_at: string; authz: { current_user_assignable_roles_for_member: string[] | null; current_user_can_remove_member: boolean } }
  ...
  const changeRole = async () => {
    changeRoleMutation.mutate(
      { teamId: params.teamId, newRole: roleChangeNewRole, memberId: roleChangeMemberId },
      {
        onSuccess: () => {
  ...
  const updateSlackStatus = async (status: boolean) => {
    updateSlackStatusMutation.mutate(
      ...
      {
        onSuccess: () => {
          toastPositive(`Slack integration ${status ? "enabled" : "disabled"} successfully`)
        },
        onError: () => {
          toastNegative(`Error ${status ? "enabling" : "disabling"} Slack integration`)
        },
      }
    )
  }
  ...
  const inviteMember = async () => {
    inviteMemberMutation.mutate(
      { teamId: params.teamId, email: inviteMemberEmail, role: inviteMemberRole },
      {
        onSuccess: () => {
        ...
        <div className="flex flex-col items-start">
          ...
          <div className="flex flex-row items-center">
            ...
            <DropdownSelect
              ...
              items={authz.can_invite_roles.map((i: string) => formatToCamelCase(i))}
              initialSelected={authz.can_invite_roles.length > 0 ? formatToCamelCase(authz.can_invite_roles[0]) : ""}
            ...
            <Button
              variant="outline"
              className="m-4"
              disabled={authz.can_invite_roles.length === 0 || inviteMemberMutation.isPending || inviteMemberEmail === ""}
          ...
          {authzAndMembersStatus === 'success' &&
            <Table className="font-display select-none table-auto w-full">
              ...
              <TableBody>
                {authz.members.map(({ id, email, role, authz: memberAuthz }: { id: string; email: string; role: string; authz: { current_user_assignable_roles_for_member: string[] | null; current_user_can_remove_member: boolean } }) => (
                  <TableRow key={id} className="font-body">
                    ...
                    {id !== currentUserId && (
                      <TableCell>
                        ...
                        {memberAuthz.current_user_assignable_roles_for_member !== null && memberAuthz.current_user_assignable_roles_for_member.length > 0 && (
                          <DropdownSelect
                            title="Roles"
                            type={DropdownSelectType.SingleString}
                            items={memberAuthz.current_user_assignable_roles_for_member.map((i: string) => formatToCamelCase(i))}
                    ...
                    {id !== currentUserId && (
                      <TableCell>
                        <Button
                          ...
                          loading={changeRoleMutation.isPending && roleChangeMemberId === id}
                          onClick={() => {
                            setRoleChangeMemberId(id)
                            setRoleChangeMemberEmail(authz.members.filter((i: AuthzMember) => i.id === id)[0].email)
                            setRoleChangeOldRole(formatToCamelCase(authz.members.filter((i: AuthzMember) => i.id === id)[0].role))
                    ...
                    {id !== currentUserId && (
                      <TableCell>
                        <Button
                          variant="outline"
                          disabled={memberAuthz.current_user_can_remove_member === false || removeMemberMutation.isPending}
                          loading={removeMemberMutation.isPending && removeMemberId === id}
                          onClick={() => {
                            setRemoveMemberId(id)
                            setRemoveMemberEmail(authz.members.filter((i: AuthzMember) => i.id === id)[0].email)
          ...
          {authzAndMembersStatus === 'success' && pendingInvitesStatus === 'success' && pendingInvites && pendingInvites.length > 0 &&
            <Table className="font-display select-none table-auto w-full">
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {pendingInvites.map(({ id, email, invited_by_email, role, valid_until }: { id: string; email: string; invited_by_email: string; role: string; valid_until: string }) => (
                  <TableRow key={id} className="font-body">
                    ...
                    <TableCell>
                      <Button
                        variant="outline"
                        disabled={!authz.can_invite_roles.includes(role) || resendPendingInviteMutation.isPending}
                        loading={resendPendingInviteMutation.isPending && resendPendingInviteId === id}
                        onClick={() => {
                    ...
                    <TableCell>
                      <Button
                        variant="outline"
                        disabled={!authz.can_invite_roles.includes(role) || removePendingInviteMutation.isPending}
                        loading={removePendingInviteMutation.isPending && removePendingInviteId === id}
                        onClick={() => {
          ...
          {slackConnectUrlStatus === 'success' && slackStatusQueryStatus === 'success' && teamSlack === null ? <a
          ...
          {slackConnectUrlStatus === 'success' && slackStatusQueryStatus === 'success' && teamSlack !== null &&
            <div className="flex flex-col w-full">
              <div className="flex flex-row w-full items-center justify-between">
                ...
                <Switch
                  disabled={!authz.can_manage_slack || updateSlackStatusMutation.isPending}
              ...
              <Button
                variant="outline"
                className="w-fit"
                disabled={!authz.can_manage_slack || testSlackAlertMutation.isPending || teamSlack.is_active === false}
          ...
          <div className="flex flex-row items-center">
            ...
            <Button
              variant="outline"
              className="m-4"
              disabled={!authz.can_rename_team || saveTeamNameButtonDisabled || changeTeamNameMutation.isPending}
