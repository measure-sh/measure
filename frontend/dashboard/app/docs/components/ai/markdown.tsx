...
// Adapted from fumadocs' ai-sdk markdown renderer template; local changes
// are import paths and the whitespace guard in rehypeWrapWords.
...
export function rehypeWrapWords() {
  return (tree: Root) => {
    visit(tree, ["text", "element"], (node, index, parent) => {
      ...
      // Whitespace-only text nodes sit between structural elements, such
      // as the newlines between a table's thead, tbody and rows. Wrapping
      // those in spans puts a <span> where HTML forbids it (a direct
      // child of <table>), which React rejects at hydration.
