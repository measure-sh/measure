...
Use this guide to set up [Measure Agent](/docs/features/feature-agent) on a self-hosted instance, so you can debug your app from your coding agent (over MCP) or from Slack.
...
The MCP endpoint is then available at `https://measure-agent.yourcompany.com/mcp`. See the [MCP Server](/docs/features/feature-mcp) guide for connecting coding agents.
