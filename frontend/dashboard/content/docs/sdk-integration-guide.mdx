...
MeasureSDK must be linked statically and will not work correctly with dynamic linking. How this is handled depends on the iOS dependency manager:

- **Swift Package Manager** (default from Flutter 3.44): `measure-sh` is linked statically by default. No extra configuration is required.
- **CocoaPods**: Flutter adds `use_frameworks!` to the iOS `Podfile` by default, which causes CocoaPods to link all pods dynamically. To fix this, follow the [CocoaPods static linking instructions](#using-cocoapods) in the iOS setup section.
