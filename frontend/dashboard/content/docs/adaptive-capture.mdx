...
| Memory collection interval           | 5 seconds                                                                           |
| Memory background collection interval (Android) | 10 seconds                                                                |
| Memory session sampling rate          | 0.01%                                                                              |
...
These settings control [memory collection](/docs/memory-monitoring). Find them under **Memory** in the app's Adaptive Capture settings.

### Collection interval

Choose how often the SDK collects memory while the app is in the foreground. The default is
5 seconds on Android and iOS. Set an interval from 1 to 3600 seconds.

### Background collection interval

_Applies only to Android._

Choose how often memory is collected while the app is in the background. The default is
10 seconds. Set an interval from 1 to 3600 seconds. iOS pauses memory collection in the background.

### Session sampling rate

Choose the percentage of sessions whose memory readings are reported, from 0% to 100%. The default
is 0.01% on Android and iOS. Sampling selects whole sessions. Foreground readings are still
collected for session replays regardless of this setting. On Android, background readings are
collected only for sampled sessions.
