...
Each report can hold up to five screenshots and a description of up to 4,000 characters entered by the user. Screenshots use the same [screenshot mask level](/docs/adaptive-capture#screenshot-mask-level) as the rest of Measure, so sensitive content stays out.
...
By default the replay covers the 5 minutes before the report. Change how far back it reaches from the dashboard through [Adaptive Capture](/docs/adaptive-capture#bug-reports).
...
Bug reports are configured remotely through [Adaptive Capture](/docs/adaptive-capture), so you can change what's collected without shipping a new build. Set the session replay duration to control how far back the replay reaches before a report, 5 minutes by default. See [Bug reports](/docs/adaptive-capture#bug-reports) for the setting.
