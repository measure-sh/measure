...
Similar errors are grouped into a single issue. For the selected time range, each group shows its number of instances, the users and sessions it affected, when it was last seen and a trend of its instances over time, so you can see at a glance what to fix first.
