          ...
          {apps.map(({ id, name, unique_identifier, platform, api_key, created_at }) => (
            <div key={id + 'app-details'} className="font-sans">
              ...
              <div key={id + 'app-api-key'} className="flex flex-row items-center">
                ...
                <button className="mx-4 my-1 outline-none flex justify-center hover:bg-yellow-200 active:bg-yellow-300 focus-visible:bg-yellow-200 border border-black rounded-md font-display transition-colors duration-100 py-2 px-4" onClick={() => navigator.clipboard.writeText(api_key.key)}>Copy</button>
              </div>
              <div className="py-8" />
            </div>
          ))}
