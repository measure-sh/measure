...
actual object Measure {
    ...
    actual fun log(
        ...
        AndroidMeasure.log(body, severity.toAndroid(), attributes.toAndroid(), timestamp)
    }

    actual fun startSpan(name: String): Span = AndroidMeasure.startSpan(name).toKmp()

    actual fun startSpan(name: String, timestamp: Long): Span = AndroidMeasure.startSpan(name, timestamp).toKmp()

    actual fun createSpanBuilder(name: String): SpanBuilder? = AndroidMeasure.createSpanBuilder(name)?.toKmp()

    actual fun getTraceParentHeaderValue(span: Span): String = AndroidMeasure.getTraceParentHeaderValue(span.unwrap())
