...
private fun Map<String, String>.toBindingsStringMap(): Map<Any?, Any> = entries.associate { (k, v) -> k to v }
...
actual object Measure {
    ...
    actual fun createSpanBuilder(name: String): SpanBuilder? = IosMeasure.createSpanBuilderWithName(name)?.let(::IosSpanBuilder)

    actual fun getTraceParentHeaderValue(span: Span): String = IosMeasure.getTraceParentHeaderValueForSpan(span.unwrap())
