...
const MetricsOverview: React.FC<MetricsOverviewProps> = ({ appId, startDate, endDate, appVersions }) => {
  ...
  return (
    <div className="flex flex-wrap gap-16 w-5/6">
      ...
      <InfoCircleExceptionRate status={metricsApiStatus} title="Perceived ANR free sessions" noData={metrics.perceived_anr_free_sessions.nan} tooltipMsgLine1="Perceived ANR free sessions = (1 - Sessions which experienced a visible ANR in selected app versions / Total sessions of selected app versions) * 100" tooltipMsgLine2="Delta value = Percieved ANR free sessions percentage of selected app versions / Perceived ANR free sessions percentage of unselected app versions" value={metrics.perceived_anr_free_sessions.perceived_anr_free_sessions} delta={metrics.perceived_anr_free_sessions.delta} />
