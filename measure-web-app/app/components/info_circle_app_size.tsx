...
const InfoCircleAppSize = ({ status, valueInBytes, deltaInBytes, title }: InfoCircleAppSizeProps) => {
  return (
    <div className="flex flex-col items-center">
      <div className={`flex flex-col group relative items-center justify-center w-64 aspect-square rounded-full border border-black border-4 hover:bg-neutral-800/25 transition-colors duration-100`}>
        ...
        {status === MetricsApiStatus.Success && <p className="font-sans text-xl"> {(valueInBytes / (1024 * 1024)).toPrecision(3)} MB</p>}
        <div className="py-1" />
        {status === MetricsApiStatus.Success && <p className={`font-sans text-sm ${deltaInBytes < 0 ? 'text-green-600' : deltaInBytes > 0 ? 'text-red-400' : ''}`}>{deltaInBytes > 0 ? '+' : ''}{(deltaInBytes / (1024 * 1024)).toPrecision(3)} MB</p>}
