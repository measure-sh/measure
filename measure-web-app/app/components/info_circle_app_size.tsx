...
const InfoCircleAppSize = ({ value, delta, title }: InfoCircleAppSizeProps) => {
    return (
      <div className="flex flex-col items-center">
           <div className={`flex flex-col group relative items-center justify-center w-64 aspect-square rounded-full border border-black border-4 hover:bg-neutral-800/25 transition-colors duration-100`}>
              <p className="text-black font-sans text-xl">{value}MB</p>
              <div className="py-1"/>
              <p className={`font-sans text-sm ${delta < 0? 'text-green-600': delta > 0? 'text-red-400': 'opacity-0'}`}>{delta>0? '+':''}{delta}MB</p>
