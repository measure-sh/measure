...
const InfoCircleAppStartTime = ({ status, value, delta, title, launchType }: InfoCircleAppStartTimeProps) => {
  return (
    <div className="flex flex-col items-center">
      <div className={`flex flex-col group relative items-center justify-center w-64 aspect-square rounded-full border border-4 transition-colors duration-100 ${status !== MetricsApiStatus.Success ? 'border-black hover:bg-neutral-800/25' : value < 800 ? 'border-green-400 hover:bg-green-400/25' : value < 1200 ? 'border-yellow-400 hover:bg-yellow-400/25' : 'border-red-400 hover:bg-red-400/25'}`}>
        ...
        {status === MetricsApiStatus.Success && <p className="font-sans text-xl"> {value}ms</p>}
        <div className="py-1" />
        {status === MetricsApiStatus.Success && <p className={`font-sans text-sm ${delta < 0 ? 'text-green-600' : delta > 0 ? 'text-red-400' : 'opacity-0'}`}>{delta > 0 ? '+' : ''}{delta}ms</p>}
