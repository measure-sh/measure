...
If you prefer to upload manually, the composed sourcemap is generated by the Gradle build at:
...
The script automatically reads the following from the Xcode build environment:
...
| dSYM path | `$DWARF_DSYM_FOLDER_PATH` |
| Build size | Size of the compiled `.app` bundle |

The script also automatically detects whether your app uses Hermes or JSC by checking the compiled bundle's binary format, and handles the upload accordingly.
