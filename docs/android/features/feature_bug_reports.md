...
* [Custom Experience](#custom-experience)
    * [Attachmemts](#attachments)
* [Benchmarks](#benchmarks) 

## Built-in Experience

Launch the default bug report interface using `Measure.launchBugReportActivity`. A screenshot is automatically taken when this method is 
...
> [!IMPORTANT]
For privacy, screenshots can be masked with the same configuration as provided during SDK initialization. See all the configuration options [here](https://github.com/measure-sh/measure/blob/main/docs/android/configuration-options.md#screenshotmasklevel).

#### Capture layout snapshot

Capture a screenshot using `captureLayoutSnapshot`. This function must be called from the main thread. Read more about layout shapshots [here](../feature_layout_snapshots.md).
