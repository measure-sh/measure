...
final class MeasureInternal { // swiftlint:disable:this type_body_length
    ...
    func log(_ body: String, severity: LogSeverity, attributes: [String: AttributeValue]) {
        ...
        logEventCollector.trackLog(body: body, severity: severity, attributes: attributes)
    }

    func log(_ body: String, severity: LogSeverity, attributes: [String: Any]) {
        guard isStarted else { return }

        let transformedAttributes = attributeTransformer.transformAttributes(attributes)

        logEventCollector.trackLog(body: body, severity: severity, attributes: transformedAttributes)
    }
