...
protocol LogEventCollector {
    func enable()
    func disable()
    func trackLog(body: String, severity: LogSeverity, attributes: [String: AttributeValue], timestamp: Number?)
}
...
final class BaseLogEventCollector: LogEventCollector {
    ...
    func trackLog(body: String, severity: LogSeverity, attributes: [String: AttributeValue], timestamp: Number?) {
        ...
        guard severity.severityNumber >= configProvider.minLogSeverityNumber else { return }
        ...
        let data = LogData(severityText: severity.severityText,
                           ...
                           body: String(body.prefix(Int(configProvider.maxLogMessageLength))))
