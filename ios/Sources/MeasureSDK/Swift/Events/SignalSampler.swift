...
final class BaseSignalSampler: SignalSampler {
    ...
    func shouldTrackMemoryUsageForSession(sessionId: String) -> Bool {
        ...
        let rate = configProvider.memoryUsageSessionSamplingRate
        guard rate.isFinite, rate > 0 else { return false }
        if rate >= 100 {
            return true
        }

        return stableSamplingValue(sessionId: sessionId) < rate / 100
    }
    ...
    func shouldSampleError(_ severity: ExceptionSeverity) -> Bool {
        ...
        switch severity {
        case .fatal:
            samplingRate = configProvider.errorFatalSamplingRate
        case .unhandled:
            samplingRate = configProvider.errorUnhandledSamplingRate
        case .handled:
            samplingRate = configProvider.errorHandledSamplingRate
        }
