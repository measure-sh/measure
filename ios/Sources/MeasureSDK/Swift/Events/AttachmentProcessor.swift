...
final class BaseAttachmentProcessor: AttachmentProcessor {
    ...
    func getAttachmentObject(for image: Data,
        ...
        case .fileStorage:
            guard let fileURL = fileManager.saveFile(data: image,
                                                     name: attachmentName,
                                                     folderName: "\(ConfigFileConstants.folderName)/\(ConfigFileConstants.attachmentsFolderName)",
                                                     directory: .applicationSupportDirectory) else {
                logger.internalLog(level: .error,
                                   message: "AttachmentProcessor: Failed to save compressed image to file storage.",
                                   error: nil,
                                   data: nil)
                return nil
            }
