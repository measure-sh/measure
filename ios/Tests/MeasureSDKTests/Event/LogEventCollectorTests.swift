...
final class BaseLogEventCollectorTests: XCTestCase {
    ...
    func testTrackLog_whenEnabled_sendsToProcessor() {
        ...
        logEventCollector.trackLog(body: "something happened", severity: .error, attributes: [:])
    ...
    func testTrackLog_withAttributes_serializesUserDefinedAttributes() {
        ...
        logEventCollector.trackLog(body: "something happened", severity: .info, attributes: attributes)
    ...
    func testTrackLog_whenDisabled_doesNotSendToProcessor() {
        ...
        logEventCollector.trackLog(body: "something happened", severity: .info, attributes: [:])
    ...
    func testTrackLog_whenMessageIsEmpty_logsWarning() {
        ...
        logEventCollector.trackLog(body: "", severity: .info, attributes: [:])
    ...
    func testTrackLog_whenMessageExceedsMaxLength_truncatesMessage() {
        logEventCollector.enable()
        let longMessage = String(repeating: "a", count: Int(configProvider.maxLogBodyLength) + 1)

        logEventCollector.trackLog(body: longMessage, severity: .info, attributes: [:])

        guard let data = signalProcessor.data as? LogData else {
            XCTFail("Expected signalProcessor.data to be of type LogData")
            return
        }
        XCTAssertEqual(data.body.count, Int(configProvider.maxLogBodyLength))
    }

    func testTrackLog_usesCurrentTime() {
        logEventCollector.enable()
    ...
    func testTrackLog_whenSeverityBelowMinLevel_doesNotSendToProcessor() {
        ...
        logEventCollector.trackLog(body: "debug message", severity: .debug, attributes: [:])
        logEventCollector.trackLog(body: "info message", severity: .info, attributes: [:])
    ...
    func testTrackLog_whenSeverityAtOrAboveMinLevel_sendsToProcessor() {
        ...
        logEventCollector.trackLog(body: "warning message", severity: .warning, attributes: [:])
    ...
    func testTrackLog_whenBodyMatchesDiscardPattern_doesNotSendToProcessor() {
        configProvider.logIgnorePatterns = ["secret"]
        logEventCollector.enable()

        logEventCollector.trackLog(body: "this contains a secret value", severity: .error, attributes: [:])
