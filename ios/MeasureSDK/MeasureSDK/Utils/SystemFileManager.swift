...
final class BaseSystemFileManager: SystemFileManager {
    ...
    private var cacheDirectory: URL? {
        guard let cacheDirectory = fileManager.urls(for: .cachesDirectory, in: .userDomainMask).first else {
            logger.internalLog(level: .error, message: "Unable to access cache directory", error: nil)
            return nil
        }
    ...
    func getCrashFilePath() -> URL? {
        ...
        do {
            ...
            logger.internalLog(level: .error, message: "Failed to create crash report directory.", error: error)
            return nil
        }
