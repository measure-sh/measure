    ...
    func insertSession(_ session: SessionEntity) {
        ...
        context.perform { [weak self] in
            ...
            do {
                ...
                self.logger.internalLog(level: .error, message: "Failed to save session: \(session.sessionId)", error: error)
            }
        }
    }
    ...
    func getSession(byId sessionId: String) -> SessionEntity? {
        ...
        context.performAndWait { [weak self] in
            do {
                ...
                guard let self = self else { return }
                logger.internalLog(level: .error, message: "Failed to fetch session: \(sessionId)", error: error)
            }
        }
        return session
    }
    ...
    func deleteSession(_ sessionId: String) {
        ...
        context.perform { [weak self] in
            do {
                ...
                guard let self = self else { return }
                self.logger.internalLog(level: .error, message: "Failed to delete session: \(sessionId)", error: error)
            }
        }
    }
    ...
    func markCrashedSessions(sessionIds: [String]) {
        ...
        context.perform { [weak self] in
            do {
                ...
                guard let self = self else { return }
                self.logger.internalLog(level: .error, message: "Failed to mark crashed sessions.", error: error)
            }
        }
    }
    ...
    func markCrashedSession(sessionId: String) {
        ...
        context.perform { [weak self] in
            do {
                ...
                guard let self = self else { return }
                self.logger.internalLog(level: .error, message: "Failed to mark crashed session: \(sessionId)", error: error)
            }
        }
    }
    ...
    func getOldestSession() -> String? {
        ...
        context.performAndWait {
            do {
                ...
                logger.internalLog(level: .error, message: "Failed to fetch oldest session.", error: error)
            }
        }
    ...
    func deleteSessions(_ sessionIds: [String]) {
        ...
        context.perform { [weak self] in
            do {
                ...
                guard let self = self else { return }
                self.logger.internalLog(level: .error, message: "Failed to delete sessions: \(sessionIds.joined(separator: ","))", error: error)
            }
        }
    }
}
