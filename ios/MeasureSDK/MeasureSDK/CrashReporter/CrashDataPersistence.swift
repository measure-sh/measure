...
final class BaseCrashDataPersistence: CrashDataPersistence {
    ...
    func prepareCrashFile() {
        if let crashFilePath = systemFileManager.getCrashFilePath() {
            ...
            if crashFileDescriptor == -1 {
                logger.internalLog(level: .error, message: "Failed to open crash log file at \(crashFilePath.path)", error: nil)
            }
        }
    }
        ...
        if let crashFilePath = systemFileManager.getCrashFilePath() {
            ...
            if fileManager.fileExists(atPath: crashFilePath.path) {
                ...
                if fileDescriptor != -1 {
                    ...
                    logger.internalLog(level: .error, message: "Crash data file cleared at \(crashFilePath.path)", error: nil)
                } else {
                    logger.internalLog(level: .error, message: "Failed to open crash log file for truncation at \(crashFilePath.path)", error: nil)
                }
            } else {
                logger.internalLog(level: .error, message: "No crash data file found to clear at \(crashFilePath.path)", error: nil)
            }
        }
    }
    ...
    func readCrashData() -> CrashDataAttributes {
        guard let crashFilePath = systemFileManager.getCrashFilePath() else {
            logger.internalLog(level: .error, message: "No crash data file found to read.", error: nil)
        ...
        do {
            ...
            guard let data = crashDataString.data(using: .utf8) else {
                logger.internalLog(level: .error, message: "Failed to convert crash data string to Data", error: nil)
                return (attribute: nil, sessionId: nil, isForeground: nil)
            }
            ...
            logger.internalLog(level: .error, message: "Failed to read or parse crash data file at \(crashFilePath.path)", error: error)
            return (attribute: nil, sessionId: nil, isForeground: nil)
        }
    }
