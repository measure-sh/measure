...
final class CrashReportingManager: CrashReportManager {
    ...
    func enableCrashReporting() {
        do {
            ...
            self.logger.internalLog(level: .info, message: "Crash reporter enabled", error: nil)
        } catch {
            self.logger.internalLog(level: .error, message: "Failed to enable crash reporter", error: error)
        }
    }
    ...
    private func processCrashReport() -> (exception: Exception?, date: Date?) {
        do {
            ...
            logger.internalLog(level: .error, message: "Error parsing crash report.", error: error)
            return (nil, nil)
        }
    }
}
