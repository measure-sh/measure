...
describe('ErrorReportingManager', () => {
  ...
  it('is idempotent — calling enable twice installs handler only once', () => {
    ...
    expect((global as any).ErrorUtils.setGlobalHandler).toHaveBeenCalledTimes(
      1
    );
  });
  ...
  it('captures a fatal exception and forwards to trackEvent', async () => {
    ...
    (global as any).ErrorUtils.getGlobalHandler.mockReturnValue(
      previousHandler
    );
    ...
    const [installedHandler] = (
      (global as any).ErrorUtils.setGlobalHandler as jest.Mock
    ).mock.calls[0];
    ...
    expect(logger.log).toHaveBeenCalledWith('fatal', 'Fatal exception', err, {
      fake: 'payload',
    });
  ...
  it('restores the original handler on disable', () => {
    ...
    const calls = ((global as any).ErrorUtils.setGlobalHandler as jest.Mock)
      .mock.calls;
